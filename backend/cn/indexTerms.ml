...
let preds_of t =
  let add_p s = function
    | IT (Apply (id, _), _, _) -> SymSet.add id s
    | _ -> s
  in
...
let rec subst (su : typed subst) (IT (it, bt, loc)) =
  ...
  | Const const ->
     IT (Const const, bt, loc)
  | Unop (uop, it) ->
     IT (Unop (uop, subst su it), bt, loc)
  | Binop (bop, t1, t2) ->
     IT (Binop (bop, subst su t1, subst su t2), bt, loc)
  | ITE (it,it',it'') ->
  ...
  | Struct (tag, members) ->
     IT (Struct (tag, map_snd (subst su) members), bt, loc)
  | StructMember (t, m) ->
     IT (StructMember (subst su t, m), bt, loc)
  | StructUpdate ((t, m), v) ->
     IT (StructUpdate ((subst su t, m), subst su v), bt, loc)
  | Record members ->
     IT (Record (map_snd (subst su) members), bt, loc)
  | RecordMember (t, m) ->
     IT (RecordMember (subst su t, m), bt, loc)
  | RecordUpdate ((t, m), v) ->
     IT (RecordUpdate ((subst su t, m), subst su v), bt, loc)
  | Cast (cbt, t) ->
     IT (Cast (cbt, subst su t), bt, loc)
  | MemberShift (t, tag, member) ->
     IT (MemberShift (subst su t, tag, member), bt, loc)
  ...
  | SizeOf t ->
     IT (SizeOf t, bt, loc)
  | OffsetOf (tag, member) ->
     IT (OffsetOf (tag, member), bt, loc)
  | Aligned t ->
     IT (Aligned {t= subst su t.t; align= subst su t.align}, bt, loc)
  | Representable (rt, t) ->
     IT (Representable (rt, subst su t), bt, loc)
  | Good (rt, t) ->
     IT (Good (rt, subst su t), bt, loc)
  | WrapI (ity, t) ->
     IT (WrapI (ity, subst su t), bt, loc)
  | Nil bt' ->
     IT (Nil bt', bt, loc)
  | Cons (it1,it2) ->
     IT (Cons (subst su it1, subst su it2), bt, loc)
  | Head it ->
     IT (Head (subst su it), bt, loc)
  | Tail it ->
     IT (Tail (subst su it), bt, loc)
  | NthList (i, xs, d) ->
     IT (NthList (subst su i, subst su xs, subst su d), bt, loc)
  | ArrayToList (arr, i, len) ->
     IT (ArrayToList (subst su arr, subst su i, subst su len), bt, loc)
  | MapConst (arg_bt, t) ->
     IT (MapConst (arg_bt, subst su t), bt, loc)
  | MapSet (t1, t2, t3) ->
     IT (MapSet (subst su t1, subst su t2, subst su t3), bt, loc)
  | MapGet (it, arg) ->
...
let is_sym = function
  | IT (Sym sym, bt, _) -> Some (sym, bt)
  | _ -> None

let is_bool = function
  | IT (Const (Bool b), _, _) -> Some b
  | _ -> None

let is_q = function
  | IT (Const (Q q), _, _) -> Some q
  | _ -> None

let is_map_get = function
  | IT (MapGet (f,arg), _, _) -> Some (f, arg)
  | _ -> None
...
let is_true = function
  | IT (Const (Bool true), _, _) -> true
  | _ -> false

let is_false = function
  | IT (Const (Bool false), _, _) -> true
  | _ -> false

let is_eq = function
  | IT (Binop (EQ, lhs, rhs), _, _) -> Some (lhs, rhs)
  | _ -> None

let is_and = function
  | IT (Binop (And, it, it'), _, _) -> Some (it, it')
  | _ -> None
...
let is_pred_ = function
  | IT (Apply (name, args), _, _) -> Some (name, args)
  | _ -> None
...
let and_sterm_ its loc = vargs_binop (bool_sterm_ true loc) (Tools.curry (fun p -> and2_sterm_ p loc)) its
let or2_sterm_ (it, it') loc = IT (Binop (Or, it, it'), SurfaceBaseTypes.Bool, loc)
let or_sterm_ its loc = vargs_binop (bool_sterm_ true loc) (Tools.curry (fun p -> or2_sterm_ p loc)) its
...
let eachI_ (i1, (s, bt), i2) t loc =
  IT (EachI ((i1, (s, bt), i2), t), BT.Bool, loc)
...
let mul_ (it, it') loc =
  if BT.equal (bt it) (bt it') then (IT (Binop (Mul,it, it'), bt it, loc))
  else failwith ("mul_: type mismatch: " ^ Pp.plain (Pp.list pp_with_typ [it; it']))
...
let record_ members loc =
  IT (Record members,
      BT.Record (List.map (fun (s,t) -> (s, basetype t)) members), loc)
...
let sizeOf_ ct loc =
  IT (SizeOf ct, Memory.size_bt, loc)
...
let array_offset_of_pointer ~base ~pointer loc =
  sub_ (pointerToIntegerCast_ pointer loc,
        pointerToIntegerCast_ base loc) loc
...
let array_pointer_to_index ~base ~item_size ~pointer loc =
  ...
  div_ (array_offset_of_pointer ~base ~pointer loc,
        item_size)
...
let subarray_condition ~base ~item_size ~from_index ~to_index ~qpointer loc =
  ...
  and_ [
      lePointer_ (pointer_offset_ (base, mul_ (item_size, from_index) loc) loc,
                  qpointer) loc;
      ltPointer_ (qpointer,
                  pointer_offset_ (base, mul_ (item_size, to_index) loc) loc) loc;
...
let mk_in_loc_list loc (ptr, opts) location = match dest_list opts with
  | Some xs -> or_sterm_ (List.map (fun x -> eq_sterm_ (ptr, x) (Cerb_location.other __FUNCTION__)) xs) location
  | None ->
    Pp.warn loc (Pp.item "cannot enumerate in_loc_list arg, treating as unspecified"
        (pp opts));
    sym_ (Sym.fresh_named "unspecified_in_loc_list", SurfaceBaseTypes.Bool, Cerb_location.other __FUNCTION__)
...
(* ct_pred *)
let representable_ (t, it) loc =
  IT (Representable (t, it), BT.Bool, loc)
let good_ (sct, it) loc =
  IT (Good (sct, it), BT.Bool, loc)
...
let const_map_ index_bt t loc =
  IT (MapConst (index_bt, t), BT.Map (index_bt, bt t), loc)
let map_set_ t1 (t2, t3) loc =
  IT (MapSet (t1, t2, t3), bt t1, loc)
...
let pred_ name args rbt loc =
  IT (Apply (name, args), rbt, loc)
...
let def_ sym e loc = eq_ (sym_ (sym, bt e, loc), e)
...
let in_z_range within (min_z, max_z) loc = match bt within with
  ...
  | BT.Bits (sign, sz) ->
    ...
    let min_c = if Z.leq min_z min_possible then bool_ true loc
      else if Z.leq min_z max_possible then le_ (num_lit_ min_z the_bt loc, within) loc
      else bool_ false loc in
    let max_c = if Z.leq max_possible max_z then bool_ true loc
...
let const_of_c_sig (c_sig : Sctypes.c_concrete_sig) loc =
  ...
  let arg_v = list_ ~item_bt:BT.CType (List.map (fun ct -> const_ctype_ ct loc) arg_cts) ~nil_loc:loc in
  Some (tuple_ [const_ctype_ ret_ct loc; arg_v;
    bool_ c_sig.sig_variadic loc; bool_ c_sig.sig_has_proto loc])))
