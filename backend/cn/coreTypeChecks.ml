...
let check_against_core_bt fail_op core_bt cn_bt =
  ...
  let fail cbt bt =
    let msg1 = Pp.typ (Pp.string "mismatching core/CN types")
        (Pp.ineq (Pp_mucore.pp_core_base_type core_bt) (BT.pp cn_bt)) in
    let msg2 = if BT.equal bt cn_bt then msg1
        else Pp.typ msg1 (Pp.typ (Pp.string "inner mismatch")
        (Pp.ineq (Pp_mucore.pp_core_base_type cbt) (BT.pp bt))) in
    fail_op msg2
  in
