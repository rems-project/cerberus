...
let [@warning "-27"] generate_c_pres_and_posts (instrumentation : Core_to_mucore.instrumentation) function_definitions =
  ...
  try
      let result =
        let open Resultat in
         ...
         let (instrumentation, _) = Core_to_mucore.collect_instrumentation prog5 in
