...
module Make (SD : sig val struct_decls : Memory.struct_decls end) : S = struct
  ...
  let of_index_term : IT.t -> Z3.Expr.expr =

    let rec term : IT.t -> Z3.Expr.expr =
    ...
    and option_op it bt = 
      ...
      | Something it ->
         ...
         let t = term it in
         Z3.Expr.mk_app context (List.hd constructors) [t]
      | Nothing _ ->
         let option_sort = sort_of_bt bt in
         let constructors = Z3.Datatype.get_constructors option_sort in
      ...
      | Is_some it -> 
         ...
         let t = term it in
         Z3.Expr.mk_app context (List.hd recognisers) [t]
      | Value_of_some it -> 
         let option_sort = sort_of_bt (IT.bt it) in
         let accessors = Z3.Datatype.get_accessors option_sort in
         let t = term it in
         Z3.Expr.mk_app context (List.hd (List.hd accessors)) [t]

    and array_op it bt = 
      match it with
      | Const t ->
         let t = term t in
         Z3.Z3Array.mk_const_array context (sort_of_bt Integer) t
      | Mod (t1, t2, t3) ->
         let t1 = term t1 in
         let t2 = term t2 in
         let t3 = term t3 in
  ...
  let rec make_trigger = function
    ...
    | T_Term it -> 
       ...
       let t1 = of_index_term t1 in
       let t2 = of_index_term it in
    ...
    | Forall ((s, bt), trigger, body) ->
       let (triggers, cs) = match trigger with
         ...
         | None ->
            ([], [])
       in
       let body = of_index_term body in
       let q = 
