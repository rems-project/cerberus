...
let state ctxt log model_with_q extras =
  ...
  let invalid_resources =
    ...
    let cht_todo = { original = !^""; simplified =[]} in
    let check (rt, o) =
      match predicate_name rt with
      | Owned _ -> cht_todo (*CHT TODO*)
      | PName s ->
        match (SymMap.find_opt s defs), o with
        | Some def, O it -> (match combine_results (check_pred def it g) with
          | No -> pp_res None (rt, o)
          | Error e -> pp_res (Some e) (rt, o)
          | _ -> cht_todo) (*CHT TODO*)
        | None, _ -> cht_todo (*CHT TODO*)
        in
    let invalid = List.filter (fun r -> not (List.is_empty r.simplified && Pp.is_empty r.original)) (List.map check (get_rs ctxt)) in
    let interesting = invalid
    in
    let uninteresting = [] in
    add_labeled
      lab_interesting
      interesting
