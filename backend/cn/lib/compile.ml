...
let register_cn_predicates env (defs : cn_predicate list) =
  let aux env def =
    ...
    let output = SBT.proj (translate_cn_base_type env (snd def.cn_pred_output)) in
  ...
  let aux env def =
    let args =
      List.map
        ...
        def.cn_func_args
    in
    let return_bt = SBT.proj (translate_cn_base_type env def.cn_func_return_bty) in
...
let add_datatype_info env (dt : cn_datatype) =
  ...
  let add_constr env (cname, params) =
    let params =
      List.map (fun (nm, ty) -> (nm, SBT.proj (translate_cn_base_type env ty))) params
    in
  ...
  UsingLoads.handle
    ...
    (let open Effectful.Make (E) in
     ...
     | CN_to_from_bytes (to_from, res) ->
       ...
       return (M_CN_statement (loc, M_CN_to_from_bytes (to_from, res)))
     | CN_have assrt ->
