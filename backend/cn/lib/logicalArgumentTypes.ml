...
(* CHT: relevant for top-level ifs *)
type 'i t =
  | Define of (Sym.t * IT.t) * info * 'i t (* CHT: let binding some index term to some symbol *)
  | Resource of (Sym.t * (RET.t * BT.t)) * info * 'i t (* CHT: takes *)
  | Constraint of LC.t * info * 'i t (* CHT: assert, etc. *)
  | I of 'i
