...
type cn_statement =
  ...
  | M_CN_to_from_bytes of CF.Cn.to_from * ResourceTypes.resource_type
  ...
  | M_CN_statement (loc, stmt) ->
    let stmt =
      ...
      | M_CN_to_from_bytes (to_from, res) ->
        M_CN_to_from_bytes (to_from, RET.subst substitution res)
      ...
      | M_CN_split_case lc -> M_CN_split_case (LC.subst substitution lc)
      ...
      | M_CN_assert lc -> M_CN_assert (LC.subst substitution lc)
      | M_CN_inline nms -> M_CN_inline nms
      | M_CN_print it -> M_CN_print (IT.subst substitution it)
    in
    M_CN_statement (loc, stmt)
...
let dtree_of_cn_statement = function
  ...
  | M_CN_to_from_bytes (To, res) -> Dnode (pp_ctor "To_bytes", [ ResourceTypes.dtree res ])
  ...
  | M_CN_split_case lc -> Dnode (pp_ctor "Split_case", [ LC.dtree lc ])
  | M_CN_extract (attrs, to_extract, it) ->
    Dnode
      ( pp_ctor "Extract",
  ...
  | M_CN_assert lc -> Dnode (pp_ctor "Assert", [ LC.dtree lc ])
  | M_CN_inline nms -> Dnode (pp_ctor "Inline", List.map (fun nm -> Dleaf (Sym.pp nm)) nms)
  | M_CN_print it -> Dnode (pp_ctor "Print", [ IT.dtree it ])
...
let rec dtree = function
  ...
  | M_CN_statement (_loc, stmt) -> dtree_of_cn_statement stmt
