...
module Debug = struct
  let dump_frame (f : solver_frame) =
    ...
    let dump_sym k v rest =
      rest ^/^ bar ^^^ Sym.pp k ^^^ !^"|->" ^^^ !^(to_string v)
    in
  ...
  let dump_solver solver =
    !^"\n|~~~~~~ Start Solver Dump ~~~~~~~|"
    ^/^ separate_map hardline dump_frame
          (!(solver.cur_frame) :: !(solver.prev_frames))
    ^/^ !^"|~~~~~~ End Solver Dump ~~~~~~~~~|"
end
...
let debug_ack_command s cmd =
  try SMT.ack_command s.smt_solver cmd
  with SMT.UnexpectedSolverResponse r ->
...
module CN_Pointer = struct
  ...
  let width =
    match Memory.uintptr_bt with
    | Bits (_, w) -> w
    | _ -> failwith "Pointer is not bits"
  ...
  let match_ptr scrutinee ~null_case ~alloc_id_addr_case =
    SMT.(
      match_datatype scrutinee
        [
          (PCon (null_name, []), null_case);
          ( PCon (alloc_id_addr_name, [ alloc_id_name; addr_name ]),
            alloc_id_addr_case ~alloc_id:(SMT.atom alloc_id_name)
  ...
  let declare s =
    ...
    ack_command s
      (SMT.define_fun addr_of_name
         [ ("p", t) ]
         (SMT.t_bits width)
         (match_ptr (SMT.atom "p") ~null_case:(SMT.bv_k width Z.zero)
...
and get_value gs ctys bt (sexp : SMT.sexp) =
  ...
  | MemByte -> (
      ...
      | con, [ salloc_id; svalue ]
        when String.equal con CN_MemByte.alloc_id_value_name ->
          let alloc_id = CN_AllocId.from_sexp salloc_id in
          let value =
            match
              get_value gs ctys (BT.Bits (Unsigned, CN_MemByte.width)) svalue
            with
            | Const (Bits (_, z)) -> z
  ...
  | Loc () -> (
      ...
      | con, [ sbase; saddr ]
        when String.equal con CN_Pointer.alloc_id_addr_name ->
  ...
  | CType -> (
      ...
      with Not_found -> Const (Default bt))
  | List elT -> (
      match SMT.to_con sexp with
  ...
  | Datatype tag -> (
      ...
      in
      let try_con c =
        if String.equal con (CN_Names.datatype_con_name c) then Some (do_con c)
        else None
      in
...
let translate_const s co =
  ...
  | Default t ->
      declare_bt_uninterpreted s CN_Constant.default t []
        (translate_base_type t)
...
let bv_clz result_w =
  ...
  let rec count w e =
    if w = 1 then SMT.ite (eq_0 w e) (result Z.one) (result Z.zero)
    else
      let top_w = w / 2 in
...
let rec translate_term s iterm =
  ...
  | Unop (op, e1) -> (
      ...
      | BW_FLS_NoSMT ->
          ...
          (* NOTE: This desugaring duplicates e1 *)
          let sz =
            match IT.bt e1 with Bits (_sign, n) -> n | _ -> assert false
          in
          let intl i = int_lit_ i (IT.bt e1) loc in
          translate_term s
            (ite_
  ...
  | Binop (op, e1, e2) -> (
      ...
      | Exp -> (
          ...
          | Some z1, Some z2 when Z.fits_int z2 ->
              translate_term s
                (num_lit_ (Z.pow z1 (Z.to_int z2)) (IT.bt e1) loc)
          | _, _ -> failwith "Exp")
  ...
  | EachI ((i1, (x, bt), i2), t) ->
      ...
      in
      if i1 > i2 then translate_term s (IT.bool_ true loc)
        ...
        List.map
          (fun (member', sct) ->
            let value =
              ...
              else member_ ~member_bt:(Memory.bt_of_sct sct) (t, member') loc
            in
            (member', value))
          members
      in
  ...
  | MemberShift (t, tag, member) ->
      CN_Pointer.ptr_shift ~ptr:(translate_term s t)
        ...
        ~offset:
          (translate_term s
             (IT (OffsetOf (tag, member), Memory.uintptr_bt, loc)))
      ...
      CN_Pointer.ptr_shift ~ptr:(translate_term s base)
        ...
        ~offset:
          (let el_size =
           ...
           let ix =
             if BT.equal (IT.bt index) Memory.uintptr_bt then index
             else cast_ Memory.uintptr_bt index loc
           in
           translate_term s (mul_ (el_size, ix) loc))
  | CopyAllocId { addr; loc } ->
      CN_Pointer.copy_alloc_id ~ptr:(translate_term s loc)
        ~null_case:(default (Loc ())) ~addr:(translate_term s addr)
  | HasAllocId loc ->
  ...
  | ArrayToList (x, y, z) ->
      ...
      let res_t = translate_base_type bt in
      let f =
        declare_bt_uninterpreted s CN_Constant.array_to_list bt arg_ts res_t
      in
      SMT.app f args
  | SizeOf ct ->
      translate_term s
        (IT.int_lit_ (Memory.size_of_ctype ct) (IT.basetype iterm) loc)
  | Representable (ct, t) ->
      translate_term s (representable struct_decls ct t loc)
  ...
  | MapSet (mp, k, v) ->
      SMT.arr_store (translate_term s mp) (translate_term s k)
        (translate_term s v)
  ...
  | Apply (name, args) -> (
      ...
      match def.body with
      | Def body ->
          translate_term s (Definition.Function.open_ def.args body args)
      | _ ->
  ...
  | Match (e1, alts) ->
      let rec match_pat v (Pat (pat, _, _)) =
        ...
        | PConstructor (c, fs) ->
            ...
            let nested_cond =
              SMT.bool_ands (List.filter_map (fun x -> x) conds)
            in
  ...
  | WrapI (ity, arg) ->
      bv_cast
        ...
        ~from:(IT.bt arg) (translate_term s arg)
  | Cast (cbt, t) -> (
      let smt_term = translate_term s t in
      match (IT.bt t, cbt) with
      | Bits _, Loc () ->
          let addr =
            if BT.equal (IT.bt t) Memory.uintptr_bt then smt_term
            else bv_cast ~to_:Memory.uintptr_bt ~from:(IT.bt t) smt_term
          in
          CN_Pointer.bits_to_ptr ~bits:addr ~alloc_id:(default Alloc_id)
      | Loc (), Bits _ ->
          let maybe_cast x =
            if BT.equal cbt Memory.uintptr_bt then x
            else bv_cast ~to_:cbt ~from:Memory.uintptr_bt x
          in
      ...
      | MemByte, Bits _ ->
          let maybe_cast x =
            if BT.equal cbt (BT.Bits (Unsigned, 8)) then x
            else bv_cast ~to_:cbt ~from:(BT.Bits (Unsigned, 8)) x
          in
...
let declare_datatype_group s names =
  let mk_con_field (l, t) =
    (CN_Names.datatype_field_name l, translate_base_type t)
  in
  let mk_con c =
...
module Logger = struct
  ...
  let make prefix =
    ...
    let get_file suf =
      ...
      open_out
        (Filename.concat dir (prefix ^ suf ^ string_of_int log_id ^ ".smt"))
    in
    if !to_file then
...
let make globals =
  ...
  | None -> ());
  (match !solver_flags with
  | Some opts -> cfg := { !cfg with SMT.opts }
  | None -> ());
  cfg :=
    {
      !cfg with
...
type model_state = Model of model_with_q | No_model

let model_state = ref No_model

let model () =
  match !model_state with No_model -> assert false | Model mo -> mo
