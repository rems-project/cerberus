...
(* TODO: DCM: I suspect one way to proceed might be to have pp.ml be just include PPrint (or delete it and get users to rename it wherever it is used). Everything else that's currently in pp.ml could be turned into pp_utils.ml *)
...
val soft_surround :
  int -> int -> document -> document -> document -> document
val surround_separate :
  int ->
  int ->
  document -> document -> document -> document -> document list -> document
val surround_separate_map :
  int ->
  int ->
  document ->
  document -> document -> document -> ('a -> document) -> 'a list -> document
val ( !^ ) : string -> document
val ( ^/^ ) : document -> document -> document
val ( ^//^ ) : document -> document -> document
...
val ( ^^^ ) :
  ...
  Cerb_pp_prelude.P.document -> Cerb_pp_prelude.P.document
...
val time_f_logs :
  Locations.t -> int -> string -> ('a -> 'b) -> 'a -> 'b
