  ...
  = (fun e es d ->
      (match es with
        | start_evaluation_scope -> 
          ...
          (* TODO: Make general *)
          let ds, s, ail_expr = cn_to_ail_expr_aux const_prop e PassBack in
          let e_cur_nm =
          match ail_expr with
  ...
  match expr_ with
    ...
    *)
    | CNExpr_memberof (e, xs) -> 
      let ds, s, e_ = cn_to_ail_expr_aux const_prop e PassBack in
    ...
    *)
    ...
    | CNExpr_binop (bop, x, y) -> 
      let ds1, s1, e1 = cn_to_ail_expr_aux const_prop x PassBack in
      let ds2, s2, e2 = cn_to_ail_expr_aux const_prop y PassBack in
    ...
    | CNExpr_cast (bt, expr) -> 
      let ds, s, e = cn_to_ail_expr_aux const_prop expr PassBack in
      let ail_expr_ = A.(AilEcast (empty_qualifiers, C.Ctype ([], cn_to_ail_base_type bt) , (mk_expr e))) in 
      dest d (ds, s, ail_expr_)
    
    | CNExpr_call (sym, exprs) -> 
      let list_of_triples = List.map (fun e -> cn_to_ail_expr_aux const_prop e PassBack) exprs in
    ...
    | CNExpr_each (sym, (r_start, r_end), e) -> 
      ...
      let list_of_triples = List.map (fun cn_const -> cn_to_ail_expr_aux (Some (sym, cn_const)) e PassBack) cn_consts in
    ...
    | CNExpr_match (e, es) -> 
      let (d1, s1, e1) = cn_to_ail_expr_aux const_prop e PassBack in
      ...
      let e1_transformed = 
        ...
        CF.Symbol.Identifier (Location_ocaml.unknown, "tag"))
      )
      in
      let (cases, exprs) = List.split es in
      let lhs = List.map (fun e_ -> cn_to_ail_expr_aux const_prop e_ PassBack) cases in
      let bindings = [] in
        ...
        (match (lhs, rhs) with
          ...
          | ((d2, s2, e2) :: ls, (ds, (s :: ss)) :: rs) ->  
            ...
            let tag_name = (match e2 with
              | A.AilEident tag_sym -> Sym.pp_string tag_sym
              | _ -> failwith "Can only pattern match on datatype constructors"
            )
            in
            (* Tag name stored in attribute *)
            let attribute : CF.Annot.attribute = {attr_ns = None; attr_id = CF.Symbol.Identifier (Location_ocaml.unknown, tag_name); attr_args = []} in
            let ail_case_stmt = A.(AnnotatedStatement (Location_ocaml.unknown, CF.Annot.Attrs [attribute], ail_case)) in
            (ail_case_stmt :: (List.map mk_stmt ss)) @ generate_switch_stats ls rs
          | _ -> failwith "Wrong pattern")  
      in
      (match d with 
        | Assert -> 
          let rhs = List.map (fun e_ -> cn_to_ail_expr_aux const_prop e_ Assert) exprs in 
          let switch = A.(AilSswitch (mk_expr e1_transformed, mk_stmt (AilSblock (bindings, generate_switch_stats lhs rhs)))) in
          (d1, s1 @ [switch])
        | Return -> 
          let rhs = List.map (fun e_ -> cn_to_ail_expr_aux const_prop e_ Return) exprs in 
          let switch = A.(AilSswitch (mk_expr e1_transformed, mk_stmt (AilSblock (bindings, generate_switch_stats lhs rhs)))) in
          (d1, s1 @ [switch])
    ...
    | CNExpr_ite (e1, e2, e3) -> 
        let d1, s1, e1_ = cn_to_ail_expr_aux const_prop e1 PassBack in
        let d2, s2, e2_ = cn_to_ail_expr_aux const_prop e2 PassBack in
        let d3, s3, e3_ = cn_to_ail_expr_aux const_prop e3 PassBack in
    ...
    *)
    ...
    | CNExpr_unchanged e -> 
      ...
      let ds, s, e_ = cn_to_ail_expr_aux const_prop (CNExpr (loc, CNExpr_binop (CN_equal, e, e_at_start))) PassBack in 
      dest d (ds, s, e_)
...
let cn_to_ail_expr
  : type a. _ cn_expr -> a dest -> a
  = fun cn_expr d ->
    cn_to_ail_expr_aux None cn_expr d
...
let cn_to_ail_datatype ?(first=false) (cn_datatype : cn_datatype) =
  ...
  in
  let generate_struct_definition (constructor, members) = (constructor, (empty_attributes, generate_tag_definition members))
  in
  let structs = List.map generate_struct_definition cn_datatype.cn_dt_cases in
  let structs = if first then 
    let generic_dt_struct = 
      (generic_cn_dt_sym, (empty_attributes, C.(StructDef (extra_members, None))))
    in
  ...
  let rec generate_stats cases count =
    (match cases with 
      ...
      | (constructor, _) :: cs -> 
        ...
        (constructor_sym, const) :: (generate_stats cs (count + 1))
  )
  in
  (* TODO: Make const? *)
...
let cn_to_ail_assertion = function
  | CN_assert_exp e_ -> 
      ...
      let _ds, ss = cn_to_ail_expr e_ Assert in 
      List.map mk_stmt ss
...
let cn_to_ail_condition cn_condition type_map = 
  ...
  | CN_cletExpr (_, name, expr) -> 
    ...
    let ds_, ss = cn_to_ail_expr expr (AssignVar name) in
