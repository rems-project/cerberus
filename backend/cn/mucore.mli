(* Module Mucore - CN-specific variant of Cerberus Core
   ...
   (Among a few other differences, Core can pass around C types as
   values – CN is more restrictive, for simplicity.)
*)

(* TODO: BCP: Not actually clear to me that this module needs a
   signature -- it's enormous, but mostly types and small functions,
   and everything it defines is used elsewhere. *)
...
type trusted = Trusted of Cerb_location.t | Checked
...
module T :
  sig
    ...
    type logical_arguments =
        Sym.t * (Sym.t * BaseTypes.t) list
    type resource_predicates =
        (Sym.t * ResourcePredicates.definition) list
    type logical_predicates =
        (Sym.t * LogicalFunctions.definition) list
  end

type symbol = Cerb_frontend.Symbol.sym
type act = { loc : loc; annot : Cerb_frontend.Annot.annot list; ct : T.ct; }
type 'TY mu_object_value_ =
  ...
  | M_OVstruct of symbol *
      (Cerb_frontend.Symbol.identifier * T.ct *
       Cerb_frontend.Impl_mem.mem_value)
      list
  | M_OVunion of symbol * Cerb_frontend.Symbol.identifier *
      Cerb_frontend.Impl_mem.mem_value
...
type mu_ctor = M_Cnil of T.cbt | M_Ccons | M_Ctuple | M_Carray
type 'TY mu_pattern_ =
...
type mu_function =
  ...
  | M_F_min_int
  | M_F_ctype_width
type bw_binop = M_BW_OR | M_BW_AND | M_BW_XOR
type bw_unop = M_BW_COMPL | M_BW_CTZ | M_BW_FFS
type bound_kind = M_Bound_Wrap of act | M_Bound_Except of act
val bound_kind_act : bound_kind -> act
type 'TY mu_pexpr_ =
    M_PEsym of symbol
  | M_PEval of 'TY mu_value
  | M_PEconstrained of
      (Cerb_frontend.Mem.mem_iv_constraint * 'TY mu_pexpr) list
  ...
  | M_PEmember_shift of 'TY mu_pexpr * symbol *
  ...
  | M_PEstruct of symbol *
      (Cerb_frontend.Symbol.identifier * 'TY mu_pexpr) list
  ...
  | M_PEbounded_binop of bound_kind * Cerb_frontend.Core.iop * 'TY mu_pexpr *
      'TY mu_pexpr
  | M_PEis_representable_integer of 'TY mu_pexpr * act
  | M_PEundef of Cerb_location.t *
      Cerb_frontend.Undefined.undefined_behaviour
...
and 'TY mu_pexpr =
    M_Pexpr of loc * Cerb_frontend.Annot.annot list * 'TY * 'TY mu_pexpr_
val loc_of_pexpr : 'a mu_pexpr -> loc
type m_kill_kind = M_Dynamic | M_Static of T.ct
type 'TY mu_action_ =
    M_Create of 'TY mu_pexpr * act * Cerb_frontend.Symbol.prefix
  | M_CreateReadOnly of 'TY mu_pexpr * act * 'TY mu_pexpr *
      Cerb_frontend.Symbol.prefix
  | M_Alloc of 'TY mu_pexpr * 'TY mu_pexpr * Cerb_frontend.Symbol.prefix
  | M_Kill of m_kill_kind * 'TY mu_pexpr
  | M_Store of bool * act * 'TY mu_pexpr * 'TY mu_pexpr *
      Cerb_frontend.Cmm_csem.memory_order
  | M_Load of act * 'TY mu_pexpr * Cerb_frontend.Cmm_csem.memory_order
  | M_RMW of act * 'TY mu_pexpr * 'TY mu_pexpr * 'TY mu_pexpr *
      Cerb_frontend.Cmm_csem.memory_order *
      Cerb_frontend.Cmm_csem.memory_order
  | M_Fence of Cerb_frontend.Cmm_csem.memory_order
  | M_CompareExchangeStrong of act * 'TY mu_pexpr * 'TY mu_pexpr *
      'TY mu_pexpr * Cerb_frontend.Cmm_csem.memory_order *
      Cerb_frontend.Cmm_csem.memory_order
  | M_CompareExchangeWeak of act * 'TY mu_pexpr * 'TY mu_pexpr *
      'TY mu_pexpr * Cerb_frontend.Cmm_csem.memory_order *
      Cerb_frontend.Cmm_csem.memory_order
  | M_LinuxFence of Cerb_frontend.Linux.linux_memory_order
  | M_LinuxLoad of act * 'TY mu_pexpr *
      Cerb_frontend.Linux.linux_memory_order
  | M_LinuxStore of act * 'TY mu_pexpr * 'TY mu_pexpr *
      Cerb_frontend.Linux.linux_memory_order
  | M_LinuxRMW of act * 'TY mu_pexpr * 'TY mu_pexpr *
      Cerb_frontend.Linux.linux_memory_order
type 'TY mu_action = M_Action of Cerb_location.t * 'TY mu_action_
type 'TY mu_paction =
    M_Paction of Cerb_frontend.Core.polarity * 'TY mu_action
type 'TY mu_memop =
  ...
  | M_PtrMemberShift of
      (symbol * Cerb_frontend.Symbol.identifier * 'TY mu_pexpr)
...
type 'TY mu_expr_ =
  ...
  | M_CN_progs of
      ...
      Cerb_frontend.Cn.cn_statement list * Cnprog.cn_prog list
and 'TY mu_expr =
    M_Expr of loc * Cerb_frontend.Annot.annot list * 'TY * 'TY mu_expr_
val loc_of_expr : 'a mu_expr -> loc
type info = Locations.info
type 'i mu_arguments_l =
    M_Define of (Sym.t * IndexTerms.t) * info *
      'i mu_arguments_l
  | M_Resource of
      (Sym.t * (ResourceTypes.t * BaseTypes.t)) *
      info * 'i mu_arguments_l
  | M_Constraint of LogicalConstraints.t * info * 'i mu_arguments_l
  | M_I of 'i
val dtree_of_mu_arguments_l :
  ('a -> Cerb_frontend.Pp_ast.doc_tree) ->
  'a mu_arguments_l -> Cerb_frontend.Pp_ast.doc_tree
type 'i mu_arguments =
    M_Computational of (Sym.t * T.bt) * info * 'i mu_arguments
  | M_L of 'i mu_arguments_l
val dtree_of_mu_arguments :
  ('a -> Cerb_frontend.Pp_ast.doc_tree) ->
  'a mu_arguments -> Cerb_frontend.Pp_ast.doc_tree
val mu_count_computational : 'a mu_arguments -> int
val mDefine :
  (Sym.t * IndexTerms.t) * info ->
  'a mu_arguments_l -> 'a mu_arguments_l
val mResource :
  (Sym.t * (ResourceTypes.t * BaseTypes.t)) *
  info -> 'a mu_arguments_l -> 'a mu_arguments_l
val mConstraint :
  LogicalConstraints.t * info ->
  'a mu_arguments_l -> 'a mu_arguments_l
val mComputational :
  (Sym.t * T.bt) * info -> 'a mu_arguments -> 'a mu_arguments
val mConstraints :
...
val mResources :
  ((Sym.t * (ResourceTypes.t * BaseTypes.t)) *
   info)
  list -> 'a mu_arguments_l -> 'a mu_arguments_l
...
val mu_fun_return_type :
  mu_function ->
  'a mu_pexpr list ->
  [> `Returns_BT of Memory.BT.basetype | `Returns_Integer ]
  Option.t
...
val evaluate_fun :
  ...
  [> `Result_IT of IT.BT.basetype IT.term | `Result_Integer of Z.t ]
  Option.m
...
type 'TY mu_label_def =
    M_Return of loc
  | M_Label of loc * 'TY mu_expr mu_arguments *
      Cerb_frontend.Annot.annot list * parse_ast_label_spec
...
type 'TY mu_globs = M_GlobalDef of T.ct * 'TY mu_expr | M_GlobalDecl of T.ct
type 'TY mu_globs_map = (symbol, 'TY mu_globs) Pmap.map
type mu_tag_definition = M_StructDef of T.st | M_UnionDef of T.ut
type mu_tag_definitions =
    (Cerb_frontend.Symbol.sym, mu_tag_definition) Pmap.map
...
type 'TY mu_file = {
  ...
  mu_lemmata :
    (Sym.t *
     (Locations.t * ArgumentTypes.lemmat))
    list;
