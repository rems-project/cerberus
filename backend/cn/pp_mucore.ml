...
module Make (Config : CONFIG) = struct
  ...
  let precedence = function
    ...
    | M_PEop (OpRem_f, _, _) ->
      Some 2
    | M_PEop (OpAdd, _, _) | M_PEop (OpSub, _, _) -> Some 3
    | M_PEop (OpLt, _, _) | M_PEop (OpLe, _, _) -> Some 4
    | M_PEop (OpGt, _, _) | M_PEop (OpGe, _, _) -> Some 4
    | M_PEop (OpEq, _, _) -> Some 5
    | M_PEop (OpAnd, _, _) -> Some 6
    | M_PEop (OpOr, _, _) -> Some 7
    | M_PEval _ | M_PEconstrained _ | M_PEsym _ | M_PEctor _ | M_PEarray_shift _
    | M_PEmember_shift _ | M_PEnot _ | M_PEstruct _ | M_PEunion _ | M_PEcfunction _
    | M_PEmemberof _ | M_PEconv_int _ | M_PEconv_loaded_int _ | M_PEwrapI _ | M_PElet _
    | M_PEif _ | M_PEundef _ | M_PEerror _
  ...
  let precedence_expr = function
    | M_Epure _ | M_Ememop _ | M_Eaction _ | M_Eskip | M_Eccall _ | M_Eunseq _
    | M_CN_progs _ ->
      None
    | M_Ebound _ -> None
    | M_End _ -> None
  ...
  let rec pp_object_value (M_OV (_bty, ov)) =
    match ov with
    | M_OVinteger ival -> Impl_mem.pp_integer_value_for_core ival
    | M_OVfloating fval ->
      Impl_mem.case_fval
        fval
        (fun () -> !^"unspec(floating)")
        (fun fval -> !^(string_of_float fval))
    | M_OVpointer ptr_val -> Impl_mem.pp_pointer_value ptr_val
    | M_OVarray lvals ->
  ...
  and pp_value (M_V (_bty, v)) =
    match v with
    | M_Vunit -> pp_const "Unit"
    | M_Vtrue -> pp_const "True"
    | M_Vfalse -> pp_const "False"
    | M_Vlist (_, cvals) -> P.brackets (comma_list pp_value cvals)
    | M_Vtuple cvals -> P.parens (comma_list pp_value cvals)
    | M_Vobject oval -> pp_object_value oval
    | M_Vctype ct -> P.squotes (pp_ctype ct)
    | M_Vfunction_addr sym -> P.parens (P.ampersand ^^^ Sym.pp sym)
  ...
  let rec pp_pattern (M_Pattern (_, _, _, pat)) =
    match pat with
    | M_CaseBase (None, bTy) -> P.underscore ^^ P.colon ^^^ pp_core_base_type bTy
    | M_CaseBase (Some sym, bTy) -> pp_symbol sym ^^ P.colon ^^^ pp_core_base_type bTy
    | M_CaseCtor (M_Ctuple, pats) -> P.parens (comma_list pp_pattern pats)
    | M_CaseCtor (ctor, pats) -> pp_ctor ctor ^^ P.parens (comma_list pp_pattern pats)
      ...
      | _ ->
        ...
        ^^^ (if compare_precedence prec' prec then fun z -> z else P.parens)
              (match pe with
                 ...
                 ^^ P.parens
                      (comma_list
                         (fun (cs, pe) ->
                           ...
                           ^^ pp_pexpr pe)
                         xs)
               | M_PEsym sym -> pp_symbol sym
               | M_PEctor (M_Cnil _, _) -> P.brackets P.empty
               | M_PEctor (M_Ctuple, pes) -> P.parens (comma_list pp_pexpr pes)
               | M_PEctor (ctor, pes) ->
               ...
               | M_Cfvfromint p1 -> !^"Cfvfromint" ^^ P.parens (pp_pexpr p1)
               ...
               | M_PEmember_shift (pe, tag_sym, Symbol.Identifier (_, memb_ident)) ->
                 ...
                 ^^ P.parens
                      (pp_pexpr pe
                       ...
                       ^^^ P.dot
                       ^^ !^memb_ident)
               | M_PEnot pe -> pp_keyword "not" ^^ P.parens (pp_pexpr pe)
               | M_PEop (bop, pe1, pe2) -> pp_pexpr pe1 ^^^ pp_binop bop ^^^ pp_pexpr pe2
               ...
               | M_PEcfunction pe -> pp_keyword "cfunction" ^^ P.parens (pp pe)
               ...
               | M_PEundef (_, ub) -> pp_keyword "undef" ^^ P.parens (pp_undef ub)
               | M_PEerror (str, pe) ->
                 pp_keyword "error"
  ...
  let pp_action budget act =
    ...
    | M_Kill (M_Dynamic, e) -> pp_keyword "free" ^^ P.parens (pp_pexpr e)
    ...
    | M_Load (ty, e, mo) -> pp_keyword "load" ^^ pp_args [ Left ty; Right e ] mo
    | M_RMW (ty, e1, e2, e3, mo1, mo2) ->
      ...
      ^^ P.parens
           (pp_actype ty
            ...
            ^^ P.comma
            ^^^ pp_memory_order mo2)
    | M_Fence mo -> pp_keyword "fence" ^^ P.parens (pp_memory_order mo)
    ...
    | M_CompareExchangeWeak (ty, e1, e2, e3, mo1, mo2) ->
      ...
      ^^ P.parens
           (pp_actype ty
            ...
            ^^ P.comma
            ^^^ pp_memory_order mo2)
    | M_LinuxFence mo -> pp_keyword "linux_fence" ^^ P.parens (pp_linux_memory_order mo)
    ...
    let rec pp budget (M_Expr (loc, annot, _, e) : 'ty mu_expr) =
      ...
      | _ ->
        ...
        do_annots
          ...
          (maybe_print_location loc
           ^^^
           match (e : 'ty mu_expr_) with
           | M_Epure pe -> pp_keyword "pure" ^^ P.parens (pp_pexpr pe)
           | M_Ememop memop ->
             let aux memop =
               ...
               match memop with
               | M_PtrEq (sym1, sym2) -> (PtrEq, [ msym sym1; msym sym2 ])
               | M_PtrNe (sym1, sym2) -> (PtrNe, [ msym sym1; msym sym2 ])
               | M_PtrLt (sym1, sym2) -> (PtrLt, [ msym sym1; msym sym2 ])
               | M_PtrGt (sym1, sym2) -> (PtrGt, [ msym sym1; msym sym2 ])
               | M_PtrLe (sym1, sym2) -> (PtrLe, [ msym sym1; msym sym2 ])
               | M_PtrGe (sym1, sym2) -> (PtrGe, [ msym sym1; msym sym2 ])
               ...
               | M_PtrWellAligned (ct1, sym1) ->
                 (PtrWellAligned, [ mctype ct1; msym sym1 ])
               ...
               | M_Memcpy (sym1, sym2, sym3) ->
                 (Memcpy, [ msym sym1; msym sym2; msym sym3 ])
               | M_Memcmp (sym1, sym2, sym3) ->
                 (Memcmp, [ msym sym1; msym sym2; msym sym3 ])
               | M_Realloc (sym1, sym2, sym3) ->
                 (Realloc, [ msym sym1; msym sym2; msym sym3 ])
               | M_Va_start (sym1, sym2) -> (Va_start, [ msym sym1; msym sym2 ])
               | M_Va_copy sym1 -> (Va_copy, [ msym sym1 ])
               | M_Va_arg (sym1, ct1) -> (Va_arg, [ msym sym1; mctype ct1 ])
               | M_Va_end sym1 -> (Va_end, [ msym sym1 ])
               | M_CopyAllocId (sym1, sym2) -> (Copy_alloc_id, [ msym sym1; msym sym2 ])
             in
             let memop, pes = aux memop in
             pp_keyword "memop"
             ^^ P.parens
                  (Pp_mem.pp_memop memop ^^ P.comma ^^^ comma_list pp_actype_or_pexpr pes)
           | M_Eaction (M_Paction (p, M_Action (_, act))) -> pp_polarity p (pp_action act)
           | M_Eskip -> pp_keyword "skip"
           ...
           | M_Eunseq es -> pp_control "unseq" ^^ P.parens (comma_list pp es)
           | M_Elet (pat, pe1, e2) ->
             P.group
               (P.prefix
                  0
           ...
           | M_Esseq (pat, e1, e2) ->
             ...
             ^^ P.break 1
             ^^ pp e2
           | M_End es -> pp_keyword "nd" ^^ P.parens (comma_list pp es)
           | M_Ebound e -> Pp.c_app (pp_keyword "bound") [ pp e ]
           | M_Erun (sym, pes) ->
  ...
  let pp_file budget file =
    ...
    guard
      ...
      (!^"-- Aggregates"
       ...
       ^^ P.break 1
       ^^ P.break 1)
    ^^ guard show_globs (!^"-- Globals" ^^ P.break 1 ^^ pp_globs budget file.mu_globs)
