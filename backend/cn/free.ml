...
let union_map (f : 'a -> SymSet.t) (xs : 'a list) =
  List.fold_left (fun acc x -> SymSet.union (f x) acc) SymSet.empty xs


let big_union sets = union_map (fun s -> s)
...
let rec free_in_pexpr (M_Pexpr (_, _, _, pexpr_)) : SymSet.t =
  ...
  | M_PElet (pattern, pe1, pe2) ->
    SymSet.union
      (free_in_pexpr pe1)
      (SymSet.diff (free_in_pexpr pe2) (bound_by_pattern pattern))
  | M_PEif (pe, mu_pexpr1, mu_pexpr2) ->
    SymSet.union (free_in_pexpr pe) (union_map free_in_pexpr [ mu_pexpr1; mu_pexpr2 ])
...
let free_in_action (M_Action (_, action_)) =
  ...
  | M_RMW (_act, pe1, pe2, pe3, _order1, _order2) ->
    union_map free_in_pexpr [ pe1; pe2; pe3 ]
  | M_Fence _order -> SymSet.empty
  | M_CompareExchangeStrong (_act, pe1, pe2, pe3, _order1, _order2) ->
    union_map free_in_pexpr [ pe1; pe2; pe3 ]
  | M_CompareExchangeWeak (_act, pe1, pe2, pe3, _order1, order2) ->
    union_map free_in_pexpr [ pe1; pe2; pe3 ]
...
let free_in_paction = function M_Paction (_polarity, action) -> free_in_action action
...
let rec free_in_expr (M_Expr (_, _, _, expr_)) =
  ...
  | M_Ewseq (pattern, e1, e2) | M_Esseq (pattern, e1, e2) ->
    SymSet.union
      (free_in_expr e1)
  ...
  | M_Eif (pe, expr1, expr2) ->
    SymSet.union (free_in_pexpr pe) (union_map free_in_expr [ expr1; expr2 ])
