...
let fetch_eqs (tab : table) lhs =
  match ITMap.find_opt lhs tab with
  | None -> []
  | Some xs -> xs

let guard_implies guard1 guard2 = match (guard1, guard2) with
  | (_, None) -> true
...
let add_eq (guard, lhs, rhs) (tab : table) =
  if eq_is_known tab (guard, lhs, rhs) then tab
  else ITMap.add lhs ({guard; rhs} :: fetch_eqs tab lhs) tab
...
let add_lc_eqs tab (lc : LogicalConstraints.t) = match lc with
  | LogicalConstraints.T it -> add_eqs tab it
  | _ -> tab
...
let get_eq_vals (tab : table) (guard : IT.t option) (x : IT.t) =
  let rec seek known = function
    | [] -> known
    | x :: xs -> if ITSet.mem x known then seek known xs
      else seek (ITSet.add x known) (fetch_implied_eqs tab guard x @ xs)
  in
  seek ITSet.empty [x]
