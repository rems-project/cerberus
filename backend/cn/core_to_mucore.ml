...
let fn_spec_instrumentation_subst_at : IT.t Subst.t -> fn_spec_instrumentation -> fn_spec_instrumentation =
  ArgumentTypes.subst rt_stmts_subst

let fn_spec_instrumentation_subst_lat : IT.t Subst.t -> (ReturnTypes.t * statements) LogicalArgumentTypes.t -> (ReturnTypes.t * statements) LogicalArgumentTypes.t =
...
let fn_spec_instrumentation_sym_subst_at (s_replace, bt, s_with) fn_spec = 
  let subst = IT.make_subst [(s_replace, IT.sym_ (s_with, bt))] in
