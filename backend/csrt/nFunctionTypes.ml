...
let normalise ft : (t) = 
  let rec aux l r c = function
    | FT.Computational ((name,bt),ft) -> Computational ((name,bt), aux l r c ft)
    | FT.Logical ((name,ls),ft) -> aux (l@[(name,ls)]) r c ft
    | FT.Resource (re,ft) -> aux l (r@[re]) c ft
    | FT.Constraint (lc,ft) -> aux l r (c@[lc]) ft
    | FT.Return rt ->
       (L ((List.fold_right mlogical l)
                 (R ((List.fold_right mresource r)
                           (C (List.fold_right mconstraint c
                             (Return (NRT.normalise rt))))))))
  in
  aux [] [] [] ft
...
let rec unnormalise_c = function
  | Constraint (lc,t) -> FT.Constraint (lc,unnormalise_c t)
  | Return rt -> FT.Return (NRT.unnormalise rt)

let rec unnormalise_r = function
  | Resource (re,t) -> FT.Resource (re,unnormalise_r t)
  | C t -> unnormalise_c t

let rec unnormalise_l = function
  | Logical ((name,ls),t) -> FT.Logical ((name,ls),unnormalise_l t)
  | R t -> unnormalise_r t

let rec unnormalise_a = function
  | Computational ((name,bt),t) -> FT.Computational ((name,bt),unnormalise_a t)
  | L t -> unnormalise_l t
