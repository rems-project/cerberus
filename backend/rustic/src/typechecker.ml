...
let loc_of_identifier (Symbol.Identifier (loc, _)) = loc
...
| [] -> pop stys ftys gamma
| AnnotatedStatement (_, annots, AilSskip) :: sts -> check_statements stys ftys gamma sts
| AnnotatedStatement (_, _, AilSexpr e) :: sts ->
  TE_either.bind
    (check_expression stys ftys gamma e)
    (fun _ -> check_statements stys ftys gamma sts)
| AnnotatedStatement (loc, _, AilSblock (bds, sts1)) :: sts ->
...
| AnnotatedStatement (_, _, AilSreturnVoid) :: sts ->
  pop stys ftys gamma
| AnnotatedStatement (_, _, AilSdeclaration [(x, e)]) :: sts ->
  check_statements stys ftys gamma sts (* TODO: ? *)
| AnnotatedStatement (_, _, s) :: sts -> failwith "TODO: unhandled statement"
...
let collect_structs s =
  let rc_types_of_fields xs =
    let xs =
      List.map
        (fun (id, (attrs, _, cty)) ->
          let rcty =
            if is_scalar cty then RC_scalar
            else failwith_id id in
          (string_of_identifier id, rcty)) xs in
