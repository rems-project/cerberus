...
#define BLOCK_LIST_PADDING                                                               \
  ((alignof(max_align_t) -                                                               \
       ((uintptr_t)free_list_mem + sizeof(block_header)) % alignof(max_align_t)) %       \
      alignof(max_align_t))
...
static inline int fl_is_valid_tag(block_header* fl) {
  return (free_list_mem < (char*)fl) && ((char*)fl <= free_list_mem + MAX_BLOCK_INDEX) &&
...
static inline block_header* fl_prev_node(block_header* fl) {
  block_header* possible_boundary_tag =
      (block_header*)((uintptr_t)fl - sizeof(block_header));
  uintptr_t possible_prev = (uintptr_t)possible_boundary_tag -
                            fl_size(possible_boundary_tag) - sizeof(block_header);
...
static inline void fl_coalesce(free_block_header* fl) {
  ...
  if (!prev_used && !next_used) {
    ...
    fl_set_size(prev,
        fl_size(prev) + fl_size(&fl->header) + fl_size(next) + 4 * sizeof(block_header));
    ...
    fl_set_size(
        &fl->header, fl_size(&fl->header) + fl_size(next) + 2 * sizeof(block_header));
  } else if (!prev_used && next_used) {
    if (prev_free_prev) {
      prev_free->prev = fl_offset(&prev_free_prev->header);
      prev_free_prev->next = fl_offset(&prev_free->header);
...
void* cn_fl_aligned_alloc(size_t alignment, size_t size) {
  ...
  while (curr) {
    ...
    size_t back_padding =
        (alignof(block_header) - size % alignof(block_header)) % alignof(block_header);

    size_t padding =
        (alignment - (curr_addr + sizeof(block_header)) % alignment) % alignment;
    if (padding != 0) {
      ...
      if (next == NULL ||
          (fl_is_used(next) && !fl_is_used(prev) &&
...
void* cn_fl_realloc(void* p, size_t nbytes) {
  ...
  if (next != NULL && !fl_is_used(next)) {
    size_t back_padding =
        (alignof(free_block_header) - nbytes % alignof(free_block_header)) %
