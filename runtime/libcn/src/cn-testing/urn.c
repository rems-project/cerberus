...
struct replace_res replace_tree(struct int_tree* tree, uint64_t weight, uint64_t value, uint64_t index) {
    ...
    if (is_leaf(tree)) {
        struct replace_res res = (struct replace_res){
            .weightOld = tree->weight,
            .valueOld = tree->value,
    ...
    if (index < tree->left->weight) {
        ...
        struct replace_res res = replace_tree(tree->right, weight, value, (index - tree->left->weight));
...
struct uninsert_res uninsert_tree(uint8_t path, struct int_tree* tree) {
    ...
    if (is_leaf(tree)) {
        ...
        free(tree);
        return (struct uninsert_res) {
            .weight = weight, .value = value, .tree = NULL
        };
    }
