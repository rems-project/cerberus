...
ocaml_native:
	@if ! (ocamlfind query cmdliner pprint zarith angstrom >/dev/null 2>&1); then \
	  ...
	  ocamlbuild -j 4 -use-ocamlfind -pkgs unix,lem,cmdliner,pprint,yojson,angstrom,${Z3},ppx_sexp_conv,sexplib,sha,camllib,apron,apron.boxD,apron.octD,apron.polkaMPQ,apron.t1pMPQ -libs str main_.native && \
