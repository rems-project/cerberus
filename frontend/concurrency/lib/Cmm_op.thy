...
definition monCheckCommitmentOrder  :: " pre_execution \<Rightarrow> execution_witness \<Rightarrow>(action)list \<Rightarrow> action \<Rightarrow>(unit)Nondeterminism.t "  where 
     " monCheckCommitmentOrder pre1 wit committed a = ( 
...
(Nondeterminism.pick (''monStep'') uncommitted_actions) (\<lambda> a .  Nondeterminism.bindExhaustive  
...
(monPerformAction pre1 s a) (\<lambda> new_wit . 
  ...
  (let new_ex = (new_pre, new_wit, standard_relations new_pre new_wit) in     Nondeterminism.bindExhaustive (Nondeterminism.bindExhaustive (Nondeterminism.bindExhaustive (monCheckConsistency new_ex) (\<lambda> _ .  monCheckWitRestrict new_wit (List.set(incCommitted   s))(incWit   s))) (\<lambda> _ .  monCheckCommitmentOrder pre1 new_wit(incCommitted   s) a)) (\<lambda> _ .  (let new_state = ((| incWit       = new_wit, 
...
(symPerformAction s a) (\<lambda> (new_wit, values1) . 
  ...
  (let new_ex = (new_pre, new_wit, standard_relations new_pre new_wit) in     Nondeterminism.bindExhaustive (Nondeterminism.bindExhaustive (Nondeterminism.bindExhaustive (monCheckConsistency new_ex) (\<lambda> _ .  monCheckWitRestrict new_wit (List.set(symCommitted   s))(symWit   s))) (\<lambda> _ .  monCheckCommitmentOrder(symPre   s) new_wit(symCommitted   s) a)) (\<lambda> _ .  (let new_state = ( s (|
                     symWit := new_wit,
