...
module MakePp (Conf: PP_CN) = struct
  ...
  let dtree_of_cn_statement (CN_statement (_loc, stmt_)) =
    ...
    | CN_to_from_bytes (To, res, args) ->
       Dnode (pp_ctor "[CN]to_bytes", (dtree_of_cn_resource res :: List.map dtree_of_cn_expr args))
    | CN_to_from_bytes (From, res, args) ->
       Dnode (pp_ctor "[CN]from_bytes", (dtree_of_cn_resource res :: List.map dtree_of_cn_expr args))
    | CN_have assrt ->
