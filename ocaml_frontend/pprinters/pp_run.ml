...
let string_of_mem_value = function
  ...
  | Cmm_aux_old.Mobj addr   -> Boot_pprint.to_plain_string (Pp_core.pp_mem_addr addr) (* (_, x) -> Z.to_string x *)
...
(*
  ...
  in *) match t with
       ...
       | (r, Some (bs, a)) :: xs ->
           ...
           (Z.to_string a) ^ ": " ^ string_of_trace_action (Pmap.find a tact_map) ^ "\n" ^ string_of_trace tact_map xs
