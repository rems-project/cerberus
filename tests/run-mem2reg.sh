...
# Verifies that --switches const_prop,mem2reg does not change output of existing CI tests.
...
for file in "${citests[@]}"; do
  ...
  if [[ $file == *.syntax-only.c ]]; then
    $CERB --nolibc --typecheck-core --switches const_prop,mem2reg ci/$file > tmp/result 2> tmp/stderr
  else
    $CERB --nolibc --typecheck-core --exec --batch --switches const_prop,mem2reg ci/$file > tmp/result 2> tmp/stderr
  fi
  ret=$?
  ...
  report "$file [+const_prop,mem2reg]" "$file" "$ret"
done
