...
let sep0map sep g docs =
  foldmap (fun x y -> x ^^ sep ^^ y) g docs

let comma_list pp = sepmap (comma ^^ break1) pp
...
module Unicode = struct
  let enc i = text (Text.encode (Text.char i))

  let compl = enc 0x00AC
  let conj = enc 0x2227
  let disj = enc 0x2228
  let implies = enc 0x2192
  let null = enc 0x2400
  let le = enc 0x2264
  let ge = enc 0x2265
  let mapsto = enc 0x21A6
end
