...
Proof.
  ...
  destruct (ATree.elt_eq s a); subst.
    simpl in H.
    destruct H. 
...
(* inductive case *)
  ...
  unfold successors_list in *. rewrite ATree.gsspec. 
  destruct H2 as [H2 | [Heq H2]]; subst.
    destruct (ATree.elt_eq n k); subst; auto.
      apply H1 in H2. unfold successors_list in H2. rewrite H in H2. inv H2.
    destruct (ATree.elt_eq k k); subst; auto.
      congruence.
Qed.
...
Lemma propagate_succ_spec:
  forall st out n,
  let st' := propagate_succ st out n in
  (L.eq _ st'.(st_in)!!n (L.lub _ st.(st_in)!!n out)) /\
  (forall s, n <> s -> st'.(st_in)!!s = st.(st_in)!!s).
Proof.
  unfold propagate_succ; intros; simpl.
...
Definition good_unreach (res: AMap.t dt) : Prop :=
  forall n, 
  ((make_predecessors successors)!!!n = nil -> 
...
Lemma propagate_succ_list_good_unreach:
  forall scs st out,
  (forall s, In s scs -> (make_predecessors successors)!!!s <> nil) ->
...
Proof.
  ...
  intros.
  apply make_predecessors_correct in H. intro J. rewrite J in H. inv H.
Qed.

Theorem fixpoint_good_unreach: forall res, 
  fixpoint = Some res -> good_unreach res.
Proof.
  unfold fixpoint. intros res PI. pattern res. 
  eapply (PrimIter.iterate_prop _ _ step 
...
Variable bound1 bound2 : set atom.
Definition dt1 := L.t bound1.
Definition dt2 := L.t bound2.
Variable successors: ATree.t (list atom).
Variable transf1: atom -> dt1 -> dt1.
Variable transf2: atom -> dt2 -> dt2.
Variable entrypoints1: list (atom * dt1).
Variable entrypoints2: list (atom * dt2).

Variable P: dt1 -> dt2 -> Prop.
Hypothesis P_bot: P (L.bot _) (L.bot _).
Hypothesis P_lub: forall x1 x2 y1 y2, 
  P x1 x2 -> P y1 y2 -> P (L.lub _ x1 y1) (L.lub _ x2 y2).
Hypothesis P_transf: forall pc x1 x2, 
  P x1 x2 -> P (transf1 pc x1) (transf2 pc x2).
Definition P_entrypoints_aux (ep1: list (atom * dt1)) (ep2: list (atom * dt2))
  := Forall2 (fun el1 el2 => 
           match el1, el2 with
...
Hypothesis P_beq : forall x1 x2 y1 y2,
  P x1 x2 -> P y1 y2 ->
  L.beq bound1 y1 (L.lub _ y1 x1) =
  L.beq bound2 y2 (L.lub _ y2 x2).

Lemma start_state_in_invariant2 : forall ep1 ep2 
  (H:P_entrypoints_aux ep1 ep2) pc, 
  P (start_state_in bound1 ep1)!!pc (start_state_in bound2 ep2)!!pc.
Proof.
  induction ep1; destruct ep2; intros; simpl; inv H.
    ...
    assert (P (start_state_in bound1 ep1)!!pc 
              (start_state_in bound2 ep2)!!pc). 
      apply IHep1. eauto.  
...
Definition invariant2 := fun st1 st2 => 
    st1.(st_wrk bound1) = st2.(st_wrk bound2) /\
    forall pc, P (st1.(st_in bound1)!!pc) (st2.(st_in bound2)!!pc).

Lemma propagate_succ_invariant2 : forall st1 st2 v1 v2 n
  (H:invariant2 st1 st2) (H0:P v1 v2),
  invariant2 (propagate_succ bound1 st1 v1 n) (propagate_succ bound2 st2 v2 n).
Proof.
  unfold invariant2, propagate_succ. intros. 
  destruct H as [J1 J2].
  case_eq (L.beq _ (st_in bound1 st1)!!n (L.lub _ (st_in bound1 st1)!!n v1)).
    intros Hbeq.
...
Lemma propagate_succ_list_invariant2 : forall l0 st1 st2 v1 v2, 
  ...
  invariant2 (propagate_succ_list bound1 st1 v1 l0) 
             (propagate_succ_list bound2 st2 v2 l0).
Proof.
  induction l0; intros; simpl. auto.
  apply IHl0; auto.
Qed.
...
Theorem fixpoint_some2:
  forall res1 res2 pc,
  fixpoint bound1 successors transf1 entrypoints1 = Some res1 ->
  fixpoint bound2 successors transf2 entrypoints2 = Some res2 ->
  P res1!!pc res2!!pc.
Proof.
  intros res1 res2 pc Hinter1 Hinter2. 
  revert pc. 
  unfold fixpoint. pattern res1, res2.
  eapply (PrimIter.iterate_some2 _ _ _ _ 
    (step bound1 successors transf1) 
    (step bound2 successors transf2) invariant2); eauto.
    intros a1 a2 H2. unfold step. 
    destruct H2 as [J1 J2]. rewrite J1.
    destruct (NS.pick (st_wrk bound2 a2)) 
      as [[n rem] | ]; eauto.
...
Theorem fixpoint_none2_right:
  fixpoint bound2 successors transf2 entrypoints2 = None ->
  fixpoint bound1 successors transf1 entrypoints1 = None.
Proof.
  unfold fixpoint.
  intros Hinter2.
  eapply (PrimIter.iterate_none2_right _ _ _ _
    (step bound1 successors transf1) 
    (step bound2 successors transf2) invariant2); eauto.
    intros a1 a2 H2. unfold step. 
    destruct H2 as [J1 J2]. rewrite J1.
...
Theorem fixpoint_none2_left:
  fixpoint bound1 successors transf1 entrypoints1 = None ->
  fixpoint bound2 successors transf2 entrypoints2 = None.
Proof.
  unfold fixpoint.
  intros Hinter1.
  eapply (PrimIter.iterate_none2_left _ _ _ _
    (step bound1 successors transf1) 
    (step bound2 successors transf2) invariant2); eauto.
    intros a1 a2 H2. unfold step. 
    destruct H2 as [J1 J2]. rewrite J1.
...
Theorem fixpoint_some2_right:
  ...
  exists res1,
    ...
    P res1!!pc res2!!pc.
Proof.
  intros res2 pc Hfix2.
  case_eq (fixpoint bound1 successors transf1 entrypoints1).
    intros res1 Hfix1.
...
Theorem fixpoint_some2_left:
  ...
  exists res2,
    ...
    P res1!!pc res2!!pc.
Proof.
  intros res1 pc Hfix1.
  case_eq (fixpoint bound2 successors transf2 entrypoints2).
    intros res2 Hfix2.
