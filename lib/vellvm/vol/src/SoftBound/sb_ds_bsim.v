...
Definition reg_bsimulation (mi:MoreMem.meminj) TD gl (F:fdef) 
  (rm1:SBspecAux.rmetadata) (rm2:rmap) (lc1 lc2:DGVMap) : Prop :=
(forall i0 gv2, 
  lookupAL _ lc2 i0 = Some gv2 -> 
  exists gv1, 
    lookupAL _ lc1 i0 = Some gv1 /\ gv_inject mi gv1 gv2
) /\
(forall vp bv2 ev2 bgv2 egv2,
...
Definition mem_bsimulation (mi:MoreMem.meminj) TD (mgb:Values.block) 
...
(forall lc2 gl b ofs als bid0 eid0 pgv' fs F B cs tmn S Ps 
    EC v cm bgv' egv',
  wf_globals mgb gl -> 
...
Definition sbEC_bsimulates_EC_tail (TD:TargetData) Ps1 gl (mi:MoreMem.meminj) 
  ...
  | (SBspec.mkEC f1 b1 ((insn_call i0 nr ca t0 v p)::cs13) tmn1 lc1 rm1 als1, 
     Opsem.mkEC f2 b2 cs2 tmn2 lc2 als2) =>
       ...
       exists ex_ids3, exists ex_ids4, exists cs22, exists cs23, exists cs24,
         ...
         reg_bsimulation mi TD gl f1 rm1 rm2 lc1 lc2 /\
         incl ex_ids ex_ids3 /\ 
...
Definition sbEC_bsimulates_EC_head (TD:TargetData) Ps1 gl (mi:MoreMem.meminj) 
  (sbEC:SBspec.ExecutionContext) (EC:Opsem.ExecutionContext) : Prop :=
  match (sbEC, EC) with
...
Fixpoint sbECs_bsimulate_ECs_tail (TD:TargetData) Ps1 gl (mi:MoreMem.meminj) 
  (sbECs:SBspec.ECStack) (ECs:Opsem.ECStack) : Prop :=
  match sbECs, ECs with
  | nil, nil => True
  | sbEC::sbECs', EC::ECs' => 
      sbEC_bsimulates_EC_tail TD Ps1 gl mi sbEC EC /\ 
      sbECs_bsimulate_ECs_tail TD Ps1 gl mi sbECs' ECs'
  | _, _ => False
  end.

Definition sbECs_bsimulate_ECs (TD:TargetData) Ps1 gl (mi:MoreMem.meminj) 
  (sbECs:SBspec.ECStack) (ECs:Opsem.ECStack) : Prop :=
  match sbECs, ECs with
  | sbEC::sbECs', EC::ECs' => 
      sbEC_bsimulates_EC_head TD Ps1 gl mi sbEC EC /\ 
      sbECs_bsimulate_ECs_tail TD Ps1 gl mi sbECs' ECs'
  | _, _ => False
  end.

Definition sbState_bsimulates_State (mi:MoreMem.meminj) (mgb:Values.block)
...
let '(OpsemAux.mkCfg S2 TD2 Ps2 gl2 fs2) := Cfg in
  ...
  | (SBspec.mkState ECs1 M1 MM1,
     Opsem.mkState ECs2 M2) =>
      ...
      sbECs_bsimulate_ECs TD1 Ps1 gl1 mi ECs1 ECs2 /\
      gl1 = gl2 /\ 
      ftable_simulation mi fs1 fs2 /\ 
      wf_globals mgb gl1 /\
      wf_sb_mi mgb mi M1 M2 /\
      mem_bsimulation mi TD1 mgb MM1 M1 M2
  end.
