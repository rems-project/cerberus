header{*Generated by Lem from set.lem.*}
...
definition split  :: " 'a Ord_class \<Rightarrow> 'a \<Rightarrow> 'a set \<Rightarrow> 'a set*'a set "  where 
  ...
  (isLess_method   dict_Basic_classes_Ord_a) p) s, set_filter ((isGreater_method   dict_Basic_classes_Ord_a) p) s))"
...
(* If the mapping function returns Just x, x is added to the result
   list. If it returns Nothing, no element is added. *)
