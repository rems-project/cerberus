...
and of_type_expr_unnamed loc names te = 
  match te with
  | Ty_refine (Coq_ident ident,type_expr) ->
     begin match StringMap.find_opt ident names with
       | Some s -> 
          let* ((bt,size),l,r,c) = of_type_expr_named loc names s te in
          return ((s, bt,size),l,r,c)
       | None -> fail loc (Generic !^("unknown name " ^ ident))
     end
  | _ -> 
     let s = Sym.fresh () in
...
and of_type_expr_named loc names newname te =
  match te with
  | Ty_refine (coq_expr,type_expr) ->
     begin match coq_expr with
     | Coq_ident ident ->
        fail loc (Generic !^("unknown name " ^ ident))
     | Coq_all coq_term -> 
        let* ((bt,size),l,r,c) = of_type_expr_named loc names newname type_expr in
        let* (l',constr) = of_coq_term loc names coq_term in
        return ((bt,size), l @ l', r, c @ [LC.LC constr])
     end
  | Ty_ptr (ptr_kind, type_expr) ->
       (* from impl_mem *)
...
let make_fun_spec_annot loc struct_decls attrs args ret_ctype = 
  ...
  let* (names, (a, l, r, c), (rl, rr, rc), arg_rts) =
    ListM.fold_leftM (fun (names, (a, l, r, c), (rl, rr, rc), arg_rts) ((msym,_), type_expr) ->
        ...
        log_name_add s;
        let sr = Sym.fresh () in
        let* ((sa, bt, osize), l', r', c') = of_type_expr_unnamed loc names type_expr in
  ...
  let* (ra, rl, rr, rc) =
    ...
    let* ((s, bt, size), rl', rr', rc') = of_type_expr_unnamed loc names type_expr in
