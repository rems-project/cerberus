...
Module CheriMemory
  (C:Capability(Morello.AddressValueBV)
       (Morello.Flags)
       (Morello.ObjTypeBV)
       (Morello.SealType)
       (Morello.BoundsBV)
       (Morello.PermissionsBV)
  )
  (IMP: Implementation)
  (TD: TagDefs)
  <: Memory(Morello.AddressValueBV)(Morello.BoundsBV).

  Include Mem_common(Morello.AddressValueBV)(Morello.BoundsBV).
  ...
  Record mem_state_r :=
    {
      ...
      last_address : Morello.AddressValueBV.t;
      ...
      dynamic_addrs : list Morello.AddressValueBV.t;
  ...
  Definition initial_address := AddressValueBV.of_Z (HexString.to_Z "0xFFFFFFFF").
  ...
  (* base address, size *)
  | FP: Morello.AddressValueBV.t -> Z -> footprint_ind.
  ...
  Definition check_overlap a b :=
    ...
    |  FP b1 sz1, FP b2 sz2 =>
         let b1 := b1 in
         let b2 := b2 in
         if andb (AddressValueBV.eqb b1 b2) (Z.eqb sz1 sz2) then
           ExactOverlap
         else if orb
                   (Z.leb (Z.add (AddressValueBV.to_Z b1) sz1) (AddressValueBV.to_Z b2))
                   (Z.leb (Z.add (AddressValueBV.to_Z b2) sz2) (AddressValueBV.to_Z b1))
              then
                Disjoint
              else
                PartialOverlap
  ...
  Definition num_of_int x :=
    ...
    | IV n => n
    | IC is_signed c =>
        let n := AddressValueBV.to_Z (C.cap_get_value c) in
  ...
  Program Definition init_ghost_tags
    ...
    let lower_a x :=
      ...
      Z.mul q align in
    let a0 := lower_a (AddressValueBV.to_Z addr) in
    let a1 := lower_a (Z.pred (Z.add (AddressValueBV.to_Z addr) size)) in
  ...
  Definition allocator (size:Z) (align:Z) : memM (storage_instance_id * Morello.AddressValueBV.t) :=
    get >>= fun st =>
        let alloc_id := st.(next_alloc_id) in
        (
          let z := Z.sub (AddressValueBV.to_Z st.(last_address)) size in
          ...
          >>= fun addr =>
            put
              {|
                ...
                next_iota        := st.(next_iota);
                last_address     := AddressValueBV.of_Z addr;
                allocations      := st.(allocations);
                iota_map         := st.(iota_map);
                funptrmap        := st.(funptrmap);
                varargs          := st.(varargs);
                ...
                capmeta          := init_ghost_tags (AddressValueBV.of_Z addr) size st.(capmeta);
                dead_allocations := st.(dead_allocations);
                dynamic_addrs    := st.(dynamic_addrs);
                last_used        := Some alloc_id;
              |}
            ;;
            ret (alloc_id, (AddressValueBV.of_Z addr)).
  ...
  Definition resolve_function_pointer
    ...
    | FP_valid (CoqSymbol.Symbol file_dig n opt_name) =>
        ...
        | None =>
            let c := C.alloc_fun (AddressValueBV.of_Z (Z.add (AddressValueBV.to_Z initial_address) n)) in
            (match opt_name with
  ...
  Fixpoint repr
    ...
    | S fuel =>
        ...
        | MVunspecified ty =>
            ...
            ret (funptrmap, (ghost_tags (AddressValueBV.of_Z addr) sz capmeta),
        ...
        | MVinteger ity (IV n_value) =>
            ...
            ret (funptrmap, (ghost_tags (AddressValueBV.of_Z addr) (Z.of_nat (List.length bs)) capmeta), bs)
  ...
  Definition allocate_object
    ...
    let align_n' := Z.max align_n (Z.add (Z.succ (0)) (AddressValueBV.to_Z (Morello.AddressValueBV.bitwise_complement (AddressValueBV.of_Z mask)))) in
      ...
      (fun '(alloc_id, addr) =>
         ...
         (match init_opt with
          ...
          | Some mval =>  (* here we allocate an object with initiliazer *)
              ...
              st <- get ;;
              '(funptrmap, capmeta, pre_bs) <- serr2memM (repr DEFAULT_FUEL st.(funptrmap) st.(capmeta) (AddressValueBV.to_Z addr) mval) ;;
              let bs := mapi (fun i b => (Z.add (AddressValueBV.to_Z addr) (Z.of_nat i), b)) pre_bs in
              put {|
                  next_alloc_id    := st.(next_alloc_id);
                  next_iota        := st.(next_iota);
                  last_address     := st.(last_address) ;
           ...
           >>=
           (fun ro =>
              let c := C.alloc_cap addr (AddressValueBV.of_Z size_n') in
              (* Check here *)
              let c :=
                if ro then
                  let p := C.cap_get_perms c in
                  let p := Morello.PermissionsBV.perm_clear_store p in
                  let p := Morello.PermissionsBV.perm_clear_store_cap p in
                  let p := Morello.PermissionsBV.perm_clear_store_local_cap p in
  ...
  Definition allocate_region
    ...
    let size_n' := C.representable_length size_n in
    let align_n' :=
      Z.max align_n (Z.succ (AddressValueBV.to_Z (Morello.AddressValueBV.bitwise_complement (AddressValueBV.of_Z mask)))) in
      ...
      (fun '(alloc_id, addr) =>
         ...
         update
           (fun (st : mem_state) =>
              ...
              |})
         ;;
         (let c_value := C.alloc_cap addr (AddressValueBV.of_Z size_n') in
          ret (PV (Prov_some alloc_id) (PVconcrete c_value))
      )).

  Definition cap_is_null  (c : C.t) : bool :=
    Z.eqb (AddressValueBV.to_Z (C.cap_get_value c)) 0.

  Definition is_dynamic addr : memM bool :=
    get >>= fun st =>
        ret (set_mem Z_as_OT.eq_dec (AddressValueBV.to_Z addr) (List.map AddressValueBV.to_Z st.(dynamic_addrs))).
  ...
  Definition kill
    ...
    | PV (Prov_symbolic iota) (PVconcrete addr) =>
        if andb
             (cap_is_null addr)
             (CoqSwitches.has_switch CoqSwitches.SW_forbid_nullptr_free)
        then
          ...
          resolve_iota precondition iota >>=
            (fun alloc_id =>
               update (fun st =>
                         ...
                         |})
               ;;
               if CoqSwitches.has_switch SW_zap_dead_pointers then
          ...
          fun x => match x with
                   ...
                   | false =>
                       get_allocation alloc_id >>= fun alloc =>
                           if Morello.AddressValueBV.eqb (C.cap_get_value addr) alloc.(base) then
                             update
                               (fun st =>
                                  {|
                                    next_alloc_id    := st.(next_alloc_id);
  ...
  Definition wrap_cap_value (n_value : Z) : Z :=
    if andb (Z.leb n_value (AddressValueBV.to_Z C.min_vaddr)) (Z.leb n_value (AddressValueBV.to_Z C.max_vaddr))
    then n_value
    else  wrapI (AddressValueBV.to_Z C.min_vaddr) (AddressValueBV.to_Z C.max_vaddr) n_value.
                ...
                ret (provs_of_bytes bs1,
                    ...
                    | Some c_value =>
                        ...
                        if iss then
                          ...
                          let c_value := C.cap_set_value c_value (AddressValueBV.of_Z (wrap_cap_value (AddressValueBV.to_Z n_value))) in
        ...
        | CoqCtype.Pointer _ ref_ty =>
            ...
            | Some cs =>
                ...
                | Some c_value =>
                    ...
                    match ref_ty with
                    | CoqCtype.Ctype _ (CoqCtype.Function _ _ _) =>
                        let n_value := Z.sub (AddressValueBV.to_Z (C.cap_get_value c_value)) (AddressValueBV.to_Z initial_address) in
                    ...
                    | _ =>
                        let prov :=
                          ...
                          | NotValidPtrProv =>
                              match
                                find_overlaping
                                  (AddressValueBV.to_Z (C.cap_get_value c_value)) with
                              | NoAlloc => Prov_none
  ...
  Definition cap_bounds_check (bounds : BoundsBV.t)
    : Z -> Z -> bool
    :=
    let '(base, limit) := BoundsBV.to_Zs bounds in
    fun (addr : Z) (sz : Z) =>
  ...
  Definition cap_check
    ...
    if (C.get_ghost_state c).(tag_unspecified) then
      ...
      let addr :=
        Z.add (AddressValueBV.to_Z (C.cap_get_value c)) offset in
      let pcheck :=
        match intent with
        | ReadIntent =>
            Morello.PermissionsBV.has_load_perm
        | WriteIntent =>
            Morello.PermissionsBV.has_store_perm
        | CallIntent =>
            Morello.PermissionsBV.has_execute_perm
        end in
      if pcheck (C.cap_get_perms c) then
        ...
        if cap_bounds_check bounds addr sz then
          ...
          fail
            (MerrCHERI loc (CheriBoundsErr (bounds, AddressValueBV.of_Z addr, (Z.to_nat sz))))
      else
  ...
  Definition find_overlaping st addr : overlap_ind
    ...
    let (require_exposed, allow_one_past) :=
      match CoqSwitches.has_switch_pred (fun x => match x with SW_PNVI _ => true | _ => false end) with
    ...
    ZMap.fold (fun alloc_id alloc acc =>
                 let new_opt :=
                   if negb (mem alloc_id st.(dead_allocations))
                      && Z.leb (AddressValueBV.to_Z alloc.(base)) addr && Z.ltb addr (Z.add (AddressValueBV.to_Z alloc.(base)) alloc.(size)) then
                     (* PNVI-ae, PNVI-ae-udi *)
                     ...
                     then None
                          ...
                          (* PNVI-ae-udi *)
                          if Z.eqb addr (Z.add (AddressValueBV.to_Z alloc.(base)) alloc.(size))
  ...
  Definition is_within_bound
    ...
    get_allocation alloc_id >>=
      (fun (alloc : allocation) =>
         ret
           (andb
              (Z.leb (AddressValueBV.to_Z alloc.(base)) addr)
              (Z.leb
                 (Z.add addr sz)
                 (Z.add (AddressValueBV.to_Z alloc.(base)) alloc.(size))))).

  Definition device_ranges : list (Morello.AddressValueBV.t * Morello.AddressValueBV.t) :=
    [ (AddressValueBV.of_Z 0x40000000, AddressValueBV.of_Z 0x40000004)
      ; (AddressValueBV.of_Z 0xABC, AddressValueBV.of_Z 0XAC0) ].
  ...
  Definition is_within_device (ty : CoqCtype.ctype) (addr : Z) : memM bool
    ...
    ret
      (List.existsb
         (fun '(min, max) =>
            andb
              (Z.leb (AddressValueBV.to_Z min) addr)
              (Z.leb (Z.add addr sz) (AddressValueBV.to_Z max)))
         device_ranges).
    ...
    get_allocation alloc_id >>=
      (fun (alloc : allocation) =>
         ...
         | Some ty, true =>
             ...
             ret
               (negb
                  (Z.eqb addr (AddressValueBV.to_Z alloc.(base)) && (Z.eqb sz alloc.(size) && e)))
         | _, _ => ret false
         end).
  ...
  Definition load
      ...
      get >>=
        (fun (st : mem_state) =>
           ...
           mem_value_strip_err mval >>=
             (fun (mval : mem_value) =>
                (if orb
                      (CoqSwitches.has_switch (CoqSwitches.SW_PNVI AE))
                      (CoqSwitches.has_switch (CoqSwitches.SW_PNVI AE_UDI))
                ...
                ;;
                sz <- serr2memM (sizeof DEFAULT_FUEL None ty) ;;
                let fp := FP (AddressValueBV.of_Z addr) sz in
                match bs' with
                | [] =>
                    if CoqSwitches.has_switch CoqSwitches.SW_strict_reads
                    then
                      match mval with
                      | MVunspecified _ =>
    ...
    let do_load_cap
      ...
      do_load alloc_id_opt (AddressValueBV.to_Z (C.cap_get_value c)) sz
    in
    ...
    | Prov_device, PVconcrete c =>
        if cap_is_null c then
          ...
          sz <- serr2memM (sizeof DEFAULT_FUEL None ty) ;;
          is_within_device ty (AddressValueBV.to_Z (C.cap_get_value c)) >>=
            ...
            is_dead z_value >>=
              (fun (function_parameter : bool) =>
                 ...
                 then ret (FAIL (MerrAccess loc LoadAccess DeadPtr))
                   ...
                   is_within_bound z_value ty (AddressValueBV.to_Z (C.cap_get_value addr)) >>=
                     (fun (function_parameter : bool) =>
                        match function_parameter with
                        | false =>
                        ...
                        | true =>
                            ...
                            (AddressValueBV.to_Z (C.cap_get_value addr)) >>=
                              (fun (function_parameter : bool) =>
                                 match function_parameter with
                                 | true =>
  ...
  Definition store
    ...
    then fail (MerrOther "store with an ill-typed memory value")
      ...
      let do_store_cap
        ...
        let addr := AddressValueBV.to_Z (C.cap_get_value c_value) in
        ...
        put {|
          ...
          |}
        ;;
        ret (FP (AddressValueBV.of_Z addr) nsz)
      in
      match prov, ptrval_ with
      | _, PVfunction _ =>
          fail
      ...
      | Prov_device, PVconcrete addr =>
          if cap_is_null addr then
            ...
            is_within_device cty (AddressValueBV.to_Z (C.cap_get_value addr)) >>=
              (fun (x : bool) =>
                 if x
      ...
      | Prov_symbolic iota, PVconcrete addr =>
          if cap_is_null addr then
            ...
            let precondition (z_value : Z) : memM merr
              :=
              is_within_bound z_value cty (AddressValueBV.to_Z (C.cap_get_value addr)) >>=
                (fun (x : bool) =>
                   match x with
                   | false =>
      ...
      | Prov_some alloc_id, PVconcrete addr
        =>
          if cap_is_null addr then
            ...
            is_within_bound alloc_id cty (AddressValueBV.to_Z (C.cap_get_value addr))
              >>=
              (fun (x : bool) =>
                 match x with
                 | false =>
  ...
  Definition concrete_ptrval : Z -> AddressValueBV.t -> serr pointer_value :=
    fun _ _ =>
      raise
 ...
 *)
  ...
  Definition case_funsym_opt (st:mem_state) (pv:pointer_value_ind): option CoqSymbol.sym
    ...
    | PVconcrete c =>
        let n := (Z.sub (AddressValueBV.to_Z (C.cap_get_value c)) (AddressValueBV.to_Z initial_address)) in
  ...
  Definition eq_ptrval
    ...
    | PVfunction (FP_invalid c1), PVfunction (FP_invalid c2) =>
        ret (AddressValueBV.eqb (C.cap_get_value c1) (C.cap_get_value c2))
    | PVfunction (FP_valid sym), PVfunction (FP_invalid c_value)
    | PVfunction (FP_invalid c_value), PVfunction (FP_valid sym) =>
        get >>=
          (fun (st : mem_state) =>
             let n_value :=
               Z.sub (AddressValueBV.to_Z (C.cap_get_value c_value)) (AddressValueBV.to_Z initial_address)
             in
             match ZMap.find n_value st.(funptrmap) with
             | Some (file_dig, name, _) =>
  ...
  Definition lt_ptrval
    ...
    | PVconcrete addr1, PVconcrete addr2 =>
        ...
        else if CoqSwitches.has_switch CoqSwitches.SW_strict_pointer_relationals then
               match
                 prov1, prov2,
  ...
  Definition diff_ptrval
    ...
    let precond (alloc: allocation) (addr1 addr2: Z): bool
      :=
      Z.leb (AddressValueBV.to_Z alloc.(base)) addr1 &&
        Z.leb addr1 (Z.add (AddressValueBV.to_Z alloc.(base)) alloc.(size)) &&
        Z.leb (AddressValueBV.to_Z alloc.(base)) addr2 &&
        Z.leb addr2 (Z.add (AddressValueBV.to_Z alloc.(base)) alloc.(size))
    in
    ...
    if CoqSwitches.has_switch (CoqSwitches.SW_pointer_arith PERMISSIVE)
    then
      match ptrval1, ptrval2 with
      | PV _ (PVconcrete addr1), PV _ (PVconcrete addr2) =>
          valid_postcond (AddressValueBV.to_Z (C.cap_get_value addr1)) (AddressValueBV.to_Z (C.cap_get_value addr2))
      | _, _=> error_postcond
      end
    else
      match ptrval1, ptrval2 with
        ...
        PV (Prov_some alloc_id2) (PVconcrete addr2) =>
          if Z.eqb alloc_id1 alloc_id2 then
            get_allocation alloc_id1 >>=
              (fun (alloc : allocation) =>
                 if precond alloc (AddressValueBV.to_Z (C.cap_get_value addr1)) (AddressValueBV.to_Z (C.cap_get_value addr2))
                 then
                   valid_postcond (AddressValueBV.to_Z (C.cap_get_value addr1)) (AddressValueBV.to_Z (C.cap_get_value addr2))
                 else
                   error_postcond)
                   ...
                   if Z.eqb alloc_id alloc_id' then
                     get_allocation alloc_id >>=
                       (fun (alloc : allocation) =>
                          if
                            precond alloc
                              (AddressValueBV.to_Z (C.cap_get_value addr1))
                              (AddressValueBV.to_Z (C.cap_get_value addr2))
                          then
                            valid_postcond
                              (AddressValueBV.to_Z (C.cap_get_value addr1))
                              (AddressValueBV.to_Z (C.cap_get_value addr2))
  ...
  Definition isWellAligned_ptrval
    ...
    | _ =>
        ...
        | PV _ (PVconcrete addr) =>
            ...
            ret (Z.eqb (Z.modulo (AddressValueBV.to_Z (C.cap_get_value addr)) sz) 0)
        end
    end.
  ...
  Definition ptrfromint
    ...
    | _, IV n_value =>
        ...
        then ret (PV Prov_none (PVconcrete (C.cap_c0 tt)))
          ...
          let addr :=
            let dlt := Z.succ (Z.sub (AddressValueBV.to_Z C.max_vaddr) (AddressValueBV.to_Z C.min_vaddr)) in
            let r_value := Z_integerRem_f n_value dlt in
            if  Z.leb r_value (AddressValueBV.to_Z C.max_vaddr)
            then r_value
          ...
          get >>=
            (fun (st : mem_state) =>
               ...
               | DoubleAlloc alloc_id1 alloc_id2 =>
                 ...
                 (fun (prov : provenance) =>
                    let c_value := C.cap_set_value (C.cap_c0 tt) (AddressValueBV.of_Z addr) in
  ...
  Definition internal_intcast
    ...
    | IC false cap, _ =>
        let n_value := AddressValueBV.to_Z (C.cap_get_value cap) in
    ...
    | IV n_value, CoqCtype.Signed CoqCtype.Intptr_t =>
        if Z.eqb n_value 0 then
          ...
          ret (inr (IC false (C.cap_set_value c_value (AddressValueBV.of_Z n_value))))
    | IV n_value, _ =>
        ret (inr (IV (conv_int_to_ity2 n_value)))
    end.
      ...
      PVfunction
        ...
        get >>=
          (fun (st : mem_state) =>
             ...
             | _ =>
                 ret (IV (Z.add (AddressValueBV.to_Z initial_address) n_value))
             end)
    | (PVfunction (FP_invalid c_value) | PVconcrete c_value) =>
        if cap_is_null c_value then
          match ity with
          ...
          (if CoqSwitches.has_switch (CoqSwitches.SW_PNVI AE) ||
                CoqSwitches.has_switch (CoqSwitches.SW_PNVI AE_UDI)
           then
             match prov with
  ...
  Definition cap_assign_value
    ...
    | IC is_signed c_value, IV n_value =>
        ret (IC is_signed
               (C.cap_set_value
                  (if C.cap_vaddr_representable c_value  (AddressValueBV.of_Z n_value)
                   then c_value
  ...
  Definition eff_array_shift_ptrval
    ...
    | PV ((Prov_symbolic iota) as prov) (PVconcrete c_value) =>
        if cap_is_null c_value then
          ...
          let shifted_addr :=
            Z.add (AddressValueBV.to_Z (C.cap_get_value c_value))
              offset in
          let precond (z_value : Z.t) : memM bool :=
            ...
            then
              get_allocation z_value >>=
                (fun (alloc : allocation) =>
                   ret (Z.leb (AddressValueBV.to_Z alloc.(base)) shifted_addr
                        && Z.leb
                             (Z.add shifted_addr sz)
                             (Z.add (Z.add (AddressValueBV.to_Z alloc.(base)) alloc.(size)) sz)))
            else
              ret true
          in
          lookup_iota iota >>=
            (fun x =>
               ...
               | inr (alloc_id1, alloc_id2) =>
                   if negb (Z.eqb ival 0) then
                     ...
                     ;;
                     let c_value := C.cap_set_value c_value (AddressValueBV.of_Z shifted_addr) in
                     ret (PV prov (PVconcrete c_value))
                   else
                     precond alloc_id1 >>=
                       (fun (function_parameter : bool) =>
    ...
    | PV (Prov_some alloc_id) (PVconcrete c_value) =>
        let shifted_addr := Z.add (AddressValueBV.to_Z (C.cap_get_value c_value)) offset in
        if CoqSwitches.has_switch (CoqSwitches.SW_pointer_arith STRICT)
           || negb (CoqSwitches.has_switch (SW_pointer_arith PERMISSIVE))
        then
          get_allocation alloc_id >>=
            (fun (alloc : allocation) =>
               if Z.leb (AddressValueBV.to_Z alloc.(base)) shifted_addr
  ...
  Definition eff_member_shift_ptrval
    ...
    | PVconcrete c_value =>
        if cap_is_null c_value then
          ...
          let addr := AddressValueBV.to_Z (C.cap_get_value c_value) in
          let c_value := C.cap_set_value c_value (AddressValueBV.of_Z (Z.add addr offset)) in
          ret (PV prov (PVconcrete c_value))
    end.
  ...
  Definition memcpy
    ...
    let cap_addr_of_pointer_value (ptr: pointer_value) : serr Z :=
      ...
      | PV _ (PVfunction (FP_invalid c_value)) =>
          ret (AddressValueBV.to_Z (C.cap_get_value c_value))
        ...
        is_dynamic addr >>=
          (fun (function_parameter : bool) =>
             ...
             | true =>
                 get_allocation alloc_id >>=
                   (fun (alloc : allocation) =>
                      if Morello.AddressValueBV.eqb alloc.(base) addr then
                        allocate_region tid (CoqSymbol.PrefOther "realloc") align size >>=
  ...
  Definition load_string (loc: location_ocaml) (c_value: C.t) (max_len: nat) : memM string
    ...
    let fix loop max_len (acc: string) (offset: Z) : memM string :=
      ...
      | S max_len =>
          ...
          let addr := Z.add (AddressValueBV.to_Z (C.cap_get_value c_value)) offset
          in
          get >>=
            (fun st =>
          ...
          (fun (st : mem_state) =>
             ...
             | Some (funptrmap, c_value) =>
                 ...
                 | MVinteger CoqCtype.Size_t (IV n_value) =>
                     let x' := AddressValueBV.to_Z (C.cap_get_value c_value) in
                     let c_value := C.cap_narrow_bounds c_value (BoundsBV.of_Zs (x', (Z.add x' n_value)))
               ...
               | Some (funptrmap, c_value) =>
                   ...
                   | MVinteger (CoqCtype.Size_t as ity) (IV n_value)
                     =>
                       ...
                       match Morello.PermissionsBV.of_list bits with
                       | None =>
                           fail
                             (MerrOther
                                (String.append
                       ...
                       fail
                         (MerrOther
                            (String.append
                               ...
                               (String.append name "'")))
                   | Some (_, c_value) =>
                       let v_value := AddressValueBV.to_Z (C.cap_get_value c_value) in
                  ...
                  (fun (st : mem_state) =>
                     ...
                     | Some (_, c_value) =>
                         ...
                         then ret (Some (MVunspecified (CoqCtype.vaddr_t tt)))
                           ...
                           let v_value := fst (BoundsBV.to_Zs (C.cap_get_bounds c_value))
                    ...
                    (fun (st : mem_state) =>
                       ...
                       | Some (_, c_value) =>
                           ...
                           then ret (Some (MVunspecified CoqCtype.size_t))
                             ...
                             let '(base, limit) := BoundsBV.to_Zs (C.cap_get_bounds c_value) in
