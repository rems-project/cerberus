...
Program Definition list_bool_to_bv (l : list bool) : bv (N.of_nat (List.length l)) := 
 ...
 Next Obligation. intros. unfold Z.of_nat. destruct (base.length l). 
...
Module Cap <: Capability (Value) (ObjType) (SealType) (Bounds) (Permissions).
  ...
  Definition sizeof_vaddr := 8%N. (* in bytes *)
  (* Definition vaddr_bits := sizeof_vaddr * 8. *)
  Definition min_vaddr := Z_to_bv (sizeof_vaddr*8) 0.  
  Definition max_vaddr := Z_to_bv (sizeof_vaddr*8) (bv_modulus (sizeof_vaddr*8)).
...
Module MorelloCap <: Capability (ValueZ) (ObjTypeZ) (SealType) (BoundsZ) (PermissionsZ).
  Definition t := Cap.t.
  ...
  (* The flags are the top byte of the value. *)
  Definition cap_get_flags (cap:t) : Flags := 
