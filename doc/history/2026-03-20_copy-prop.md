...
These `let weak alias = pure(sym)` bindings are semantically trivial (they copy
a pure value into a fresh name with no memory or sequencing effect), but they
pollute the IR and impede other analyses.  Eliminating the aliases before
mem2reg runs would restore the full precision of that analysis.
...
- Regression script: `tests/run-copy-prop.sh`
...
| Suite | Passed | Failed |
|---|---|---|
| copy_prop (regression, 188 tests) | 188 | 0 |
