...
module Make = 
 ...
 struct 
  ...
  | Nil_stack -> f
  | Cons_stack (state_cur, s0, s1) -> f0 state_cur s0 s1 (stack_rect f f0 s1)
  ...
  | Nil_stack -> f
  | Cons_stack (state_cur, s0, s1) -> f0 state_cur s0 s1 (stack_rec f f0 s1)
  ...
  let reduce stack_cur production0 =
    bind2
      ...
      | Some s ->
        ...
        OK (Cons_stack (s,
        (uncurry (List0.map (Obj.magic __) (A.Gram.prod_rhs production0)) act
          sem), stack_new))
      | None -> Err)
    ...
    | Coq_inr awt ->
      ...
      (match awt term with
       | A.Shift_act state_new ->
         OK (Progress_sr ((Cons_stack (state_new, sem, stack_cur)),
           (tl buffer)))
