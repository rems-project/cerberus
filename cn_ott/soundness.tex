...
\begin{proof}
    ...
    \step{<1>2}{%
        \case{%
            ...
            \textsc{Ty\_Res\_Conj}, \textsc{Ty\_Spine\_Res\_Phi},
            \textsc{Ty\_PE\_AssertUndef}, \textsc{Ty\_TPVal\_\{Undef,Done\}},
            \textsc{Ty\_Action\_\{Load,Store,Kill\}},
            \textsc{Ty\_Memop\_PtrValidForDeref},
            \textsc{Ty\_TVal\_\{Phi,Undef\}}.
        }{%
            \pf\ Assume $\cnkw{smt} \, \cnsym{(}  \Phi  \Rightarrow  \cnnt{term'}  \cnsym{)}$. Show $\cnkw{smt} \, \cnsym{(}  \Phi'  \Rightarrow  \cnnt{term'}  \cnsym{)}$.By \textsc{Weak\_Cons\_Phi}, if $\cnnt{term} \in \Phi$ then
            $\cnnt{term} \in \Phi'$. Any extra constraints in $\Phi'$ (by
            \textsc{Weak\_Skip\_Phi}) would either be irrelevant, redundant, or
            inconsistent. In all cases, $\cnkw{smt} \, \cnsym{(}  \Phi'  \Rightarrow  \cnnt{term'}  \cnsym{)}$ as required.
        }
    }
...
\begin{proof}
    ...
    \step{<1>2}{%
        \case{%
            ...
            % This gets around the equivalence required by the typing rule
            % by having the pattern-matching require syntactic equality.
        }{
    ...
    \step{<1>21}{%
        \case{%
            ...
            \assume{%
                ...
                \end{pfenum}}
            \prove{%
                $\cdot  \cnsym{;}  \cdot  \cnsym{;}  \cdot  \cnsym{;}  \cdot  \cnsym{,}  \cnnt{pt}  \vdash  \cnkw{done} \, bool\_value  \cnsym{,}  \cnnt{pt}  \Leftarrow  \cnnt{ret}$.
            }
        }
    }
