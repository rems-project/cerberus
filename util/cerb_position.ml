...
module LineMap = struct
  ...
  type t = {
    file:   string;               (* preprocessed file *)
    lines:  (string * int) Map.t 
    (* The map associates lines in the (preprocessed files) with
       declarations about the original source (file, line number).
       We keep a whole map, rather than just the last locations,
       to support parsing in mulitple passes.  For example, we first pass
       CN magic comments as just a literal, and then we pre-process them
       in a secondary pass. *)
