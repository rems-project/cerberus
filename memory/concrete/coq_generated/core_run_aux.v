...
Record core_run_annotation : Type := {
  sb_before:  set  ((mem_common.thread_id  * memory_order.aid ) % type);
  dd_before:  set  memory_order.aid ;
  asw_before: set  memory_order.aid 
}.
...
Definition thread_state_default: thread_state  := {| arena := DAEMON; stack := DAEMON; errno := unit_default; env := DAEMON; exec_loc := exec_location_default; current_loc := unit_default |}.
...
Instance x77_Default : Default core_run_annotation := {
...
Program Fixpoint add_to_sb  (p_aids : set ((polarity *((mem_common.thread_id *memory_order.aid ) % type)) % type)) (g : generic_expr (core_run_annotation ) (unit ) (symbol.sym ))  : generic_expr (core_run_annotation ) (unit ) (symbol.sym ):= 
    ...
    if false then
      Expr annot1
        match ( expr_) with | Epure _ => (* no change *) expr_
          ...
          | Ewseq _as e1 e2 =>
          let is_positive := fun (x : (polarity *((mem_common.thread_id *memory_order.aid ) % type)) % type) =>
        ...
        Ewseq _as (add_to_sb p_aids e1)
          ...
          | Easeq _ _ _ => DAEMON | Ebound _ => DAEMON | End es =>
          ...
          | Ewait _ => (* no change *) expr_ | Eannot _ _ => DAEMON
          | Eexcluded _ _ => DAEMON end else expr1 end.
(* [?]: removed value specification. *)


Program Fixpoint add_to_sb_continuation  (p_aids : set ((polarity *((mem_common.thread_id *memory_order.aid ) % type)) % type))  : list (continuation_element (core_run_annotation )) -> list (continuation_element (core_run_annotation )):=  
...
Program Fixpoint add_to_sb_stack  (p_aids : set ((polarity *((mem_common.thread_id *memory_order.aid ) % type)) % type)) (sk : stack (core_run_annotation ))  : stack (core_run_annotation ):= 
...
Program Fixpoint add_to_asw  (aids : set (nat )) (g : generic_expr (core_run_annotation ) (unit ) (symbol.sym ))  : generic_expr (core_run_annotation ) (unit ) (symbol.sym ):= 
  match ( (aids,g)) with ( aids,  Expr annot1 expr_ as expr1) =>
    if false then
      match ( debug.print_debug ( 5) [debug.DB_driver; debug.DB_concurrency]
                (fun (u : unit ) =>
                               ...
                               match ( (u) ) with ( tt) =>
                                 String.append "ADD_TO_ASW ==> "
                                   (Coq.Numbers.DecimalString.NilZero.string_of_int
                                      (Z.to_int
                                         (Coq.ZArith.BinIntDef.Z.of_nat aid1)))
                               end) ) aids in
          Eaction
            (Paction p'
               (Action loc
            ...
            | Easeq _ _ _ => DAEMON | Ebound _ => DAEMON
            ...
            (* no change *) expr_ | Eannot _ _ => DAEMON | Eexcluded _ _ =>
            DAEMON end end else expr1 end.
(* [?]: removed value specification. *)


Program Fixpoint add_to_asw_continuation  (aids : set (memory_order.aid ))  : list (continuation_element (core_run_annotation )) -> list (continuation_element (core_run_annotation )):=  
  fun (x : list (continuation_element (core_run_annotation ))) =>
    match (x) with | [] => [] | Kunseq annots1 es1 es2 :: cont =>
...
Program Fixpoint add_to_asw_stack  (aids : set (memory_order.aid )) (sk : stack (core_run_annotation ))  : stack (core_run_annotation ):=  
  match ( debug.print_debug ( 5) [debug.DB_driver; debug.DB_concurrency]
            (fun (u : unit ) =>
  ...
  match ( (g)) with (( Pexpr annot1 _ pexpr_)) =>
    Pexpr annot1 tt
      match ( pexpr_) with | PEsym sym1 => PEsym sym1 | PEimpl iCst =>
        ...
        PEconstrained
          (List.map
             (fun (p : (mem_common.mem_constraint (unit )*generic_pexpr bty (symbol.sym )) % type) =>
        ...
        PEstruct sym1
          (List.map
             (fun (p : (symbol.identifier *generic_pexpr bty (symbol.sym )) % type) =>
  ...
  match ( (act_)) with ( act_) =>
    match ( act_) with | Create pe1 pe2 pref =>
      ...
      CreateReadOnly (convert_pexpr pe1) (convert_pexpr pe2)
        (convert_pexpr pe3) pref | Alloc pe1 pe2 pref =>
      Alloc (convert_pexpr pe1) (convert_pexpr pe2) pref | Kill b pe =>
      Kill b (convert_pexpr pe) | Store b pe1 pe2 pe3 mo1 =>
      Store b (convert_pexpr pe1) (convert_pexpr pe2) (convert_pexpr pe3) mo1
      | SeqRMW b pe1 pe2 sym1 pe3 =>
      SeqRMW b (convert_pexpr pe1) (convert_pexpr pe2) sym1
        (convert_pexpr pe3) | Load pe1 pe2 mo1 =>
      Load (convert_pexpr pe1) (convert_pexpr pe2) mo1
      | RMW pe1 pe2 pe3 pe4 mo1 mo2 =>
      RMW (convert_pexpr pe1) (convert_pexpr pe2) (convert_pexpr pe3)
        (convert_pexpr pe4) mo1 mo2 | Fence mo1 => Fence mo1
