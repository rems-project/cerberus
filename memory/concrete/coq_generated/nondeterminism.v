...
Definition kill_reason_default{err: Type} : kill_reason err := Undef0 err unit_default DAEMON.
...
(* HACK *)
Instance x38_Constraints : Constraints unit := {
   empty  :=  tt;
   negate   x :=  tt;
   concat   x  y :=  tt
}.
...
Definition nd_action_default {a: Type} {info: Type} {err: Type} {cs: Type} {st: Type} : nd_action a info err cs st := NDactive a info err cs st DAEMON.
Definition ndM_default {a: Type} {info: Type} {err: Type} {cs: Type} {st: Type} : ndM a info err cs st := ND a info err cs st (fun (x37 : st) => (DAEMON, DAEMON)).
...
Definition print_debug0 {cs err info st : Type}  (level : nat ) (doms : list (domain )) (msg : unit  -> string )  : ndM (unit ) info err cs st:= 
  nd_return (debug.print_debug level doms msg).
(* [?]: removed value specification. *)
...
Definition msum {a cs err info st : Type} `{Constraints cs} `{Show info}  (info1 : info)  : list ((info*ndM a info err cs st) % type) -> ndM a info err cs st:=  
  fun (x : list ((info*ndM a info err cs st) % type)) =>
    match (x) with | [] => DAEMON | [(_,  m)] => m | info_ms =>
      match ( debug.print_debug ( 1) []
                (fun (u : unit ) =>
                   match ( (u) ) with ( tt) =>
                          ...
                          (Coq.Numbers.DecimalString.NilZero.string_of_int
                             (Z.to_int
                                (Coq.ZArith.BinIntDef.Z.of_nat
                                   (List.length info_ms)))) ")") end )) with
          tt => ND (fun (st1 : st) => (NDnd info1 info_ms, st1) ) end end.
(* [?]: removed top-level value definition. *)
(* [?]: removed top-level value definition. *)
(* [?]: removed value specification. *)
...
Program Fixpoint nd_foldlM {a b cs err info st : Type}  (f : a -> b -> ndM a info err cs st) (a1 : a) (l : list b)  : ndM a info err cs st:=  
  match ( l) with 
  | [] =>
      nd_return a1
  | x::xs => nd_bind
      (f a1 x) (fun (fax : a) => nd_foldlM f fax xs)
  end.
(* [?]: removed top-level value definition. *)
(* [?]: removed value specification. *)
...
with liftAction {a cs err1 err2 info1 info2 st1 st2 : Type}  (get1 : st2 -> st1) (put1 : st2 -> st1 -> st2) (liftInfo : info1 -> info2) (liftErr : err1 -> err2) (nd : nd_action a info1 err1 cs st1)  : nd_action a info2 err2 cs st2:= 
  match ( (get1,put1,liftInfo,liftErr,nd)) with
      ( get1,  put1,  liftInfo,  liftErr,  nd) =>
    match ( nd) with | NDactive a1 => NDactive a1 | NDkilled r =>
      NDkilled
        ...
        (List.map
           (fun (p : (info1*ndM a info1 err1 cs st1) % type) =>
              match ( (p) ) with ( (info,  m_act)) =>
                (liftInfo info, liftND get1 put1 liftInfo liftErr m_act) end)
           str_ms) | NDguard info cs1 act =>
      NDguard (liftInfo info) cs1 (liftND get1 put1 liftInfo liftErr act)
      | NDbranch info cs1 m_act1 m_act2 =>
      NDbranch (liftInfo info) cs1 (liftND get1 put1 liftInfo liftErr m_act1)
...
Definition nd_status_default {a: Type} {err: Type} : nd_status a err := Active a err DAEMON.
