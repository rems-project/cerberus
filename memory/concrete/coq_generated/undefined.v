(* Generated by Lem from ocaml_generated/undefined.lem. *)
...
Require Import bimap.
Require Export bimap.
...
Definition undefined_behaviour_default: undefined_behaviour  := DUMMY string_default.


Definition undefined_behaviour_compare {a : Type}   : a -> a -> ordering :=  (fun (x : a) (y : a)=>EQ). (* TODO: CHECK *)

Instance x30_Ord : Ord undefined_behaviour := { 
   compare  :=  undefined_behaviour_compare;
   isLess   x  y :=  (ordering_equal (undefined_behaviour_compare x y) LT);
   isLessEqual   x  y :=  negb ((ordering_equal (undefined_behaviour_compare x y) GT));
   isGreater   x  y :=  (ordering_equal (undefined_behaviour_compare x y) GT);
   isGreaterEqual   x  y :=  negb ((ordering_equal (undefined_behaviour_compare x y) LT))
}.
 


Instance x29_Eq : Eq undefined_behaviour := { (* TODO: check *)
...
Definition std_of_undefined_behaviour   : undefined_behaviour  -> option (string ) :=  
  fun (x : undefined_behaviour ) =>
    match (x) with | UB004a_incorrect_main_return_type =>
      Some "Â§5.1.2.2.1#1" | UB004b_incorrect_main_argument1 =>
      Some "Â§5.1.2.2.1#1" | UB004c_incorrect_main_argument2 =>
      Some "Â§5.1.2.2.1#1" | UB004d_main_not_function => Some "Â§5.1.2.2.1#1"
      | UB005_data_race => Some "Â§5.1.2.4#25, sentence 2"
      | UB008_multiple_linkage => Some "Â§6.2.2#7"
      | UB009_outside_lifetime => Some "Â§6.2.4#2, sentence 3"
      | UB010_pointer_to_dead_object => None (* TODO *)
      | UB012_lvalue_read_trap_representation =>
      Some "Â§6.2.6.1#5, sentence 2"
      | UB013_lvalue_side_effect_trap_representation =>
      Some "Â§6.2.6.1#5, sentence 3" | UB015_incompatible_redeclaration =>
      Some "Â§6.2.7#2" | UB020_nonarray_incomplete_lvalue_conversion =>
      Some "Â§6.3.2.1#2, sentence 3" | UB038_number_of_args =>
      Some "Â§6.5.2.2#6, sentence 3" | UB041_function_not_compatible =>
      Some "Â§6.5.2.2#9" | UB043_indirection_invalid_value =>
      Some "Â§6.5.3.2#4, sentence 4"
      | UB050_pointers_subtraction_not_representable =>
      Some "Â§6.5.6#9, sentence 3"
      | UB055_invalid_integer_constant_expression => Some "Â§6.6#6"
      | UB059_incomplete_no_linkage_identifier => Some "Â§6.7#7"
      | UB060_block_scope_function_with_storage_class => Some "Â§6.7.1#7"
      | UB061_no_named_members => Some "Â§6.7.2.1#8, sentence 3"
      | UB066_qualified_function_specification =>
      Some "Â§6.7.3#9, sentence 2" | UB078_modified_void_parameter =>
      Some "Â§6.7.6.3#10" | UB081_scalar_initializer_not_single_expression =>
      Some "Â§6.7.9#11, 1st sentence"
      | UB086_incomplete_adjusted_parameter => Some "Â§6.9.1#7, sentence 4"
      | UB089_tentative_definition_internal_linkage => Some "Â§6.9.2#3"
      | UB204_illtyped_Static_assert => Some "Â§6.7.10#3"
      | UB205_atomic_store_memorder => Some "Â§7.17.7.1#2"
      | UB206_atomic_load_memorder => Some "Â§7.17.7.2#2"
      | UB207_atomic_compare_exchange_memorder => Some "Â§7.17.7.4#2" | _ =>
      (* TODO *) None end.
(* [?]: removed value specification. *)

Definition ub_str_bimap   : list ((undefined_behaviour *string ) % type):= 
  bimap.fromList0
...
Definition stringFromUndefined_behaviour   : undefined_behaviour  -> string :=  
  fun (x : undefined_behaviour ) =>
    match (x) with | DUMMY str =>
      String.append "DUMMY(" (String.append str ")") | ub =>
      match ( bimap.lookupL ub ub_str_bimap) with | Some str => str | None =>
        match ( ub) with | Invalid_format str =>
          String.append "Invalid_format[" (String.append str "]") | _ =>
          DAEMON end end end.

Instance x28_Show : Show undefined_behaviour := {
   show  :=  stringFromUndefined_behaviour
}.

(* [?]: removed value specification. *)

Definition ub_short_string   : undefined_behaviour  -> string :=  
  fun (x : undefined_behaviour ) =>
    match (x) with | UB004a_incorrect_main_return_type =>
      ...
      "variable named 'main' with external linkage has undefined behaviour"
      | UB008_multiple_linkage =>
      "identifier has both internal and external linkage in the same translation unit"
      ...
      | UB041_function_not_compatible => "incompatible function type"
      ...
      "the result of some pointer arithmetic operator was out of bound"
      | UB066_qualified_function_specification =>
      "the specification of a function type should include any type qualifiers"
      ...
      "out of bounds pointer at memory store" | ub =>
      String.append "(UB missing short message): "
        (stringFromUndefined_behaviour ub) end.
(* [?]: removed value specification. *)

Definition pretty_string_of_undefined_behaviour   : undefined_behaviour  -> string :=  
  fun (x : undefined_behaviour ) =>
    match (x) with | DUMMY str =>
      String.append "DUMMY undefined behaviour (" (String.append str ")")
      | UB_unspecified_lvalue => "(defacto) unspecified lvalue"
      | UB005_data_race =>
      "(Â§5.1.2.4#25) the execution of the program contains a data race."
      | UB008_multiple_linkage =>
      "(Â§6.2.2#7) an identifier appeared with both internal and external linkage"
      | UB009_outside_lifetime =>
      "(Â§6.2.4#2) an object is referred to outside of its lifetime."
      ...
      | UB012_lvalue_read_trap_representation =>
      "(Â§6.2.6.1#5) an lvalue expression that does not have character type tried to read from an object storing a trap representation."
      | UB013_lvalue_side_effect_trap_representation =>
      "(Â§6.2.6.1#5) an lvalue expression that does not have character type performed a side effect that produced a trap representation in (part of) an object."
      | UB014_unsupported_negative_zero =>
      "(Â§6.2.6.2#3) the evaluation of one of these operators: &, |, ^, ~, <<, and >> produced a negative zero, but this implementation being used does not support them."
      | UB015_incompatible_redeclaration =>
      "(Â§6.2.7#2) multiple declarations of a same object or function had incompatible types"
      | UB019_lvalue_not_an_object =>
      "(Â§6.3.2.1#1) found an lvalue that does not designate an object when evaluted."
      | UB020_nonarray_incomplete_lvalue_conversion =>
      "TODO(msg) UB020_nonarray_incomplete_lvalue_conversion"
      | UB022_register_array_decay => "TODO(msg) UB022_register_array_decay"
      ...
      "(Â§6.5#2) a side effect on a scalar object is unsequenced relative to either a different side effect on the same scalar object or a value computation using the value of the same scalar object."
      | UB036_exceptional_condition =>
      "(Â§6.5#2) an exceptional condition occurred during the evaluation of an expression."
      | UB037_illtyped_load =>
      ...
      "(Â§6.5.2.3#5) a member of an atomic structure or union object is accessed."
      | UB043_indirection_invalid_value =>
      "(Â§6.5.3.2#4) the operand of the unary * operator has an invalid value."
      | UB045a_division_by_zero =>
      "(Â§6.5.5#5) the value of the second operand of a / operator is zero."
      | UB045b_modulo_by_zero =>
      "(Â§6.5.5#5) the value of the second operand of a % operator is zero."
      | UB045c_quotient_not_representable =>
      "(Â§6.5.5#6) the quotient a/b is not representable."
      | UB046_array_pointer_outside =>
      "(Â§6.5.6#8) found the addition/subtraction of a pointer into, or just beyond, and array object and an integer type procuding a result that does not point into, or just beyond, the same array object."
      | UB047a_array_pointer_addition_beyond_indirection =>
      "(Â§6.5.6#8) found the addition of a pointer into, or just beyond, an array object and an integer type procuding a result that points just beyond the array object and is used as the operand of a unary * operator that is evaluated."
      | UB047b_array_pointer_subtraction_beyond_indirection =>
      "(Â§6.5.6#8) found the addition of a pointer into, or just beyond, an array object and an integer type procuding a result that points just beyond the array object and is used as the operand of a unary * operator that is evaluated."
      | UB048_disjoint_array_pointers_subtraction =>
      "(Â§6.5.6#9) found the subtraction of two pointers that do not point into, or just beyond, the same array object."
      | UB050_pointers_subtraction_not_representable =>
      "(Â§6.5.6#9) found the subtraction of two pointers whose result is not representable in an object of type ptrdiff_t"
      | UB051a_negative_shift =>
      "(Â§6.5.7#3) found a bitwise shift operator (<< >>) whose right operand has negative value."
      | UB51b_shift_too_large =>
      "(Â§6.5.7#3) found a bitwise shift operator (<< >>) whose right operand has a value larger than or equal to the width of the promoted left operand."
      | UB052a_negative_left_shift =>
      "(Â§6.5.7#4) found << operator whose operand has a negative value."
      | UB052b_non_representable_left_shift =>
      "(Â§6.5.7#4) found << operator whose operand has a value that is not representable in the promoted type."
      | UB053_distinct_aggregate_union_pointer_comparison =>
      "(Â§6.5.8#5) found the comparison of two pointers that do not point to the same aggregate or union (nor just beyond the same array object)."
      | UB059_incomplete_no_linkage_identifier =>
      "(Â§6.7#7) If an identifier for an object is declared with no linkage, the type for the object shall be complete by the end of its declarator, or by the end of its init-declarator if it has an initializer; in the case of function parameters (including in prototypes), it is the adjusted type (see 6.7.6.3) that is required to be complete."
      | UB061_no_named_members => "TODO(msg) UB061_no_named_members"
      | UB081_scalar_initializer_not_single_expression =>
      "(Â§6.7.9#11) The initializer for a scalar shall be a single expression, optionally enclosed in braces."
      | UB088_reached_end_of_function =>
      "(Â§6.9.1#12) If the } that terminates a function is reached, and the value of
 the function call is used by the caller, the behavior is undefined."
      | UB111_illtyped_assert =>
      "(Â§7.2.1.1#1) found an assert whose argument does not have a scalar type"
      | UB153a_insufficient_arguments_for_format =>
      "Insufficient_arguments_for_format"
      | UB153b_illtyped_argument_for_format => "Illtyped_argument_for_format"
      | UB204_illtyped_Static_assert =>
      "(Â§6.7.10#3) The constant expression shall be an integer constant expression."
      | UB_CERB001_integer_to_dead_pointer =>
      "(CERB notes0117 3.3.1) static assert expression is not an integral constant expression"
      | UB_CERB002a_out_of_bound_load =>
      "(CERB) provenance check failed during a load"
      | UB_CERB002b_out_of_bound_store =>
      "(CERB) provenance check failed during a store" | Invalid_format str =>
      String.append "Invalid_format """ (String.append str """") | ub =>
      match ( bimap.lookupL ub ub_str_bimap) with | Some str =>
        String.append "TODO(pretty ub) ==> " str | None =>
        "TODO(pretty ub) ==> unknown" end end.


(* Exception monad for possibly undefined executions *)
Inductive t0 (a : Type) : Type := 
  | Defined:  a -> t0 a
  | Undef:  unit  ->  list  undefined_behaviour  -> t0 a
  | Error:  unit  ->  string  -> t0 a.
Definition t0_default{a: Type} : t0 a := Defined a DAEMON.
Arguments Defined {_} _.
Arguments Undef {_} _ _.
Arguments Error {_} _ _.
(* [?]: removed value specification. *)

Definition return0 {a : Type}  (x : a)  : t0 a:=  Defined x.
(* [?]: removed value specification. *)

Definition bind0 {a b : Type}  (m : t0 a) (f : a -> t0 b)  : t0 b:= 
  match ( m) with 
    | Defined x => f x
...
Definition sequence {a : Type}  (ms : list (t0 a))  : t0 (list a):=  List.fold_right (fun (m : t0 a) (acc : t0 (list a)) =>   bind0
  m (fun (x : a)  => bind0
  acc (fun (xs : list a) =>
  return0 (x::xs)))) (return0 []) ms.
(* [?]: removed value specification. *)

Definition mapM {a b : Type}  (f : a -> t0 b) (xs : list a)  : t0 (list b):=  sequence (List.map f xs).
(* [?]: removed value specification. *)

Definition undef {a : Type}  (loc : unit ) (us : list (undefined_behaviour ))  : t0 a:=  Undef loc us.
(* [?]: removed value specification. *)

Definition error {a : Type}  (loc : unit ) (str : string )  : t0 a:=  Error loc str.
(* [?]: removed value specification. *)

Definition fmap0 {a b : Type}  (f : a -> b) (m : t0 a)  : t0 b:= 
  match ( m) with 
...
(* for convenience, for connecting with the OCaml *)
Definition ub_str_bimap_lookupL   (u : undefined_behaviour )   :  option  string  := 
  bimap.lookupL u ub_str_bimap.

Definition ub_str_bimap_lookupR   (s : string )   :  option  undefined_behaviour  := 
  bimap.lookupR s ub_str_bimap.
