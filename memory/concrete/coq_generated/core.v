...
Instance x111_Eq : Eq core_object_type := {
...
Instance x110_Eq : Eq core_base_type := {
...
Instance x107_MapKeyType{sym: Type} `{MapKeyType sym}: MapKeyType (generic_name  sym):= {
...
Instance x106_Located{bty sym: Type} : Located (generic_pattern  bty  sym):= {
   locOf   pat :=  match ( pat) with ( Pattern annots1 _) => get_loc_ annots1 end
}.


Instance x105_Located{bty sym: Type} : Located (generic_pexpr  bty  sym):= {
   locOf   pe :=  match ( pe) with ( Pexpr annots1 _ _) => get_loc_ annots1 end
}.


Instance x104_Located{a bty sym: Type} : Located (generic_expr  a  bty  sym):= {
...
Instance x103_Show : Show linking_kind := {
  show   x :=  
   match ( x) with 
   | LK_none => "LK_none"
...
Definition typed_file_default{a: Type} : typed_file a := DAEMON. (* workaround for Lem issue #74 *)
 ...
 Instance x102_SetType : SetType polarity := {
