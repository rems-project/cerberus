...
Definition ord_of_core_object_type   : core_object_type  -> nat :=  
  fun (x : core_object_type ) =>
    match (x) with | OTy_integer => ( 0 : nat ) | OTy_floating =>  1
      | OTy_pointer =>  2 | OTy_array _ =>  3 | OTy_struct _ =>  4
      | OTy_union _ =>  5 end.

Fixpoint eq_core_object_type  (oTy1 : core_object_type ) (oTy2 : core_object_type )  : bool := 
  match ( (oTy1, oTy2)) with 
    | (OTy_array oTy1,  OTy_array oTy2) =>
        eq_core_object_type oTy1 oTy2
    | (OTy_struct sym1,  OTy_struct sym2) => symbol.symbolEqual0
        sym1 sym2
    | (OTy_union sym1,  OTy_union sym2) => symbol.symbolEqual0
        sym1 sym2
    | _ => beq_nat
        (ord_of_core_object_type oTy1) (ord_of_core_object_type oTy2)
  end.
Instance x59_Eq : Eq core_object_type := {
...
Definition ord_of_core_base_type  (o : core_base_type )  : nat := 
  match ( o) with 
  | BTy_unit =>
     ( 0 : nat )
  | BTy_boolean => 1
...
Definition eq_core_base_type   : core_base_type  -> core_base_type  -> bool :=  classical_boolean_equivalence. (* CHECK *)
Instance x58_Eq : Eq core_base_type := {
...
Definition generic_name_default{sym: Type} : generic_name sym := Sym sym DAEMON.
Arguments Sym {_} _.
Arguments Impl {_} _.
 (* implementation-defined constant *)

Instance x55_MapKeyType{sym: Type} `{MapKeyType sym}: MapKeyType (generic_name  sym):= {
...
Inductive generic_object_value (sym : Type) : Type :=   (* C object values *)
 | OVinteger:  unit  -> generic_object_value sym (* integer value *)
 | OVfloating:  unit  -> generic_object_value sym (* floating-point value *)
 | OVpointer:  unit  -> generic_object_value sym (* pointer value *)
 | OVarray:  list  (generic_loaded_value  sym) -> generic_object_value sym (* C array value *)
 | OVstruct:  symbol.sym  ->  list  ((symbol.identifier  * ctype  * unit ) % type) -> generic_object_value sym (* C struct value *)
 | OVunion:  symbol.sym  ->  symbol.identifier  ->  unit  -> generic_object_value sym (* C union value *)

with generic_loaded_value (sym : Type) : Type :=   (* potentially unspecified C object values *)
  (* non-unspecified loaded value *)
 | LVspecified:  (generic_object_value  sym) -> generic_loaded_value sym 
 (* unspecified loaded value *)
 | LVunspecified:  ctype  -> generic_loaded_value sym.
Definition generic_object_value_default{sym: Type} : generic_object_value sym := OVinteger sym unit_default.
...
Definition generic_loaded_value_default{sym: Type} : generic_loaded_value sym := LVspecified sym DAEMON.
...
Definition generic_value_default{sym: Type} : generic_value sym := Vobject sym DAEMON.
...
Inductive generic_pexpr_ (bty : Type) (sym : Type) : Type :=   (* Core pure expressions *)
 ...
 | PEmember_shift:  (generic_pexpr  bty  sym) ->  sym ->  symbol.identifier  -> generic_pexpr_ bty sym (* pointer struct/union member shift *)
 ...
 | PEstruct:  sym ->  list  ((symbol.identifier  * (generic_pexpr  bty  sym)) % type) -> generic_pexpr_ bty sym (* C struct expression *)
 | PEunion:  sym ->  symbol.identifier  ->  (generic_pexpr  bty  sym) -> generic_pexpr_ bty sym (* C union expression *)
 | PEcfunction:  (generic_pexpr  bty  sym) -> generic_pexpr_ bty sym (* C function pointer expression *)
 | PEmemberof:  sym ->  symbol.identifier  ->  (generic_pexpr  bty  sym) -> generic_pexpr_ bty sym (* C struct/union member access *)
...
Inductive generic_action_ (bty : Type) (sym : Type) : Type :=   (* memory actions *)
 ...
 | Alloc:  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  symbol.prefix  -> generic_action_ bty sym
 | Kill:  bool  ->  (generic_pexpr  bty  sym) -> generic_action_ bty sym (* the boolean indicates whether the action is dynamic (i.e. free()) *)
 | Store:  bool  ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  memory_order.memory_order  -> generic_action_ bty sym (* the boolean indicates whether the store is locking *)
 | Load:  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  memory_order.memory_order  -> generic_action_ bty sym
 ...
 | RMW:  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  memory_order.memory_order  ->  memory_order.memory_order  -> generic_action_ bty sym
 | Fence:  memory_order.memory_order  -> generic_action_ bty sym
 | CompareExchangeStrong:  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  memory_order.memory_order  ->  memory_order.memory_order  -> generic_action_ bty sym
 | CompareExchangeWeak:  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  (generic_pexpr  bty  sym) ->  memory_order.memory_order  ->  memory_order.memory_order  -> generic_action_ bty sym
...
Inductive dyn_annotation : Type := 
  | DA_neg:  nat  ->  list  nat  ->  unit  -> dyn_annotation  (* the nat is its unique "exclusion ID", the list is its "exclusion set" *)
  | DA_pos:  list  nat  ->  unit  -> dyn_annotation .
Definition dyn_annotation_default: dyn_annotation  := DA_neg nat_default DAEMON unit_default. (* the list of nat is its "exclusions set" *)
...
Instance x54_Located{bty sym: Type} : Located (generic_pattern  bty  sym):= {
   locOf   pat :=  match ( pat) with ( Pattern annots1 _) => get_loc_ annots1 end
}.


Instance x53_Located{bty sym: Type} : Located (generic_pexpr  bty  sym):= {
   locOf   pe :=  match ( pe) with ( Pexpr annots1 _ _) => get_loc_ annots1 end
}.


Instance x52_Located{a bty sym: Type} : Located (generic_expr  a  bty  sym):= {
...
Instance x51_Show : Show linking_kind := {
  show   x :=  
   match ( x) with 
   | LK_none => "LK_none"
...
Definition extern_map : Type := 
  fmap  symbol.identifier   ((list  symbol.sym  * linking_kind ) % type).
...
Definition action (a: Type) : Type :=        generic_action  a  unit   symbol.sym .
Definition action_default{a: Type} : action a := DAEMON.
...
Definition typed_file_default{a: Type} : typed_file a := DAEMON. (* workaround for Lem issue #74 *)
 ...
 Instance x50_SetType : SetType polarity := {
