  ...
  match ( (c)) with (( Ctype _ ty)) =>
    match ( ty) with | Void => None | Basic( Integer _) => Some OTy_integer
      ...
      match ( core_object_type_of_ctype ty) with | Some oTy =>
        Some (OTy_array oTy) | None => DAEMON end
      | Function _ (_,  ret_ty) xs isVariadic => DAEMON | Pointer _ _ =>
      Some OTy_pointer | Atomic ty => core_object_type_of_ctype ty
      | Struct tag_sym => Some (OTy_struct tag_sym) | Union tag_sym =>
      Some (OTy_union tag_sym) end end.
(* [?]: removed value specification. *)
...
Program Fixpoint objectValueFromMemValue  (mem_val : unit )  : (core_object_type *generic_object_value (symbol.sym )) % type:=  
  match ( debug.print_debug ( 6) []
            (fun (u : unit ) =>
               match ( (u) ) with ( tt) =>
                 String.append "objectValueFromMemValue ==> " (BOOM mem_val)
               end )) with tt =>
    DAEMON mem_val (fun (ty : ctype ) => DAEMON)
      (fun ( _ : integerType ) ( _ : symbol.sym ) => DAEMON)
      (fun ( _ : integerType ) (ival : unit ) =>
         (OTy_integer, OVinteger ival))
      (fun ( _ : floatingType ) (fval : unit ) =>
         (OTy_floating, OVfloating fval))
      (fun ( _ : ctype ) (ptr_val : unit ) =>
         (OTy_pointer, OVpointer ptr_val))
      (fun (mem_vals : list (unit )) => DAEMON)
      (fun (sym1 : symbol.sym ) (xs : list ((symbol.identifier *ctype *unit ) % type)) =>
         (OTy_struct sym1, OVstruct sym1 xs))
      (fun (sym1 : symbol.sym ) (ident : symbol.identifier ) (mem_val : unit ) =>
         (OTy_union sym1, OVunion sym1 ident mem_val)) end
with loadedValueFromMemValue  (mem_val : unit )   : ( core_object_type  * loaded_value ) % type:=  
  match ( debug.print_debug ( 6) []
            (fun (u : unit ) =>
               match ( (u) ) with ( tt) =>
...
Definition valueFromMemValue  (mem_val : unit )  : (core_object_type *generic_value (symbol.sym )) % type:=  
  match ( debug.print_debug ( 6) []
            (fun (u : unit ) =>
               match ( (u) ) with ( tt) =>
      ...
      (fun (mem_vals : list (unit )) =>
         match ( lem_list.unzip (List.map loadedValueFromMemValue mem_vals)) with
           ...
           let oTy := match ( oTys) with | [] =>
                        (* Something went wrong in the MLM *) DAEMON
                        | oTy :: _ => oTy end in
...
Program Fixpoint memValueFromValue  (c : ctype ) (cval : generic_value (symbol.sym ))  : option (unit ) := 
    ...
    match ( (ty, cval)) with | (_,  Vunit) => None | (_,  Vtrue) => None
      ...
      match ( debug.print_debug ( 1) []
        ...
        if core_ctype_aux.are_compatible0
          ...
          Some (DAEMON tag_sym1 xs) else None end
      | (Union tag_sym1,  Vloaded( LVspecified( OVunion tag_sym2 ident mem_val))) =>
      if symbol.symbolEqual0 tag_sym1 tag_sym2 then
        Some (DAEMON tag_sym1 ident mem_val) else None | _ =>
      match ( debug.print_debug ( 5) []
                (fun (u : unit ) =>
                   match ( (u) ) with ( tt) =>
                     String.append "memValueFromValue("
                       (String.append (DAEMON (Ctype [] ty))
                          (String.append ", "
                             (String.append (DAEMON cval) ")"))) end)) with
          tt => None end end end.
(* [?]: removed value specification. *)
...
Definition mk_tuple_pat   : list (generic_pattern (unit ) (symbol.sym )) -> generic_pattern (unit ) (symbol.sym ):=  
  fun (x : list (generic_pattern (unit ) (symbol.sym ))) =>
    match (x) with | [] => DAEMON | [pat] => pat | pats =>
      Pattern [] (CaseCtor Ctuple pats) end.
(* [?]: removed value specification. *)
...
Definition mk_integer_pe  (n : Z )  : generic_pexpr (unit ) (symbol.sym ):= 
  Pexpr [] tt (PEval (Vobject (OVinteger (DAEMON n)))).
(* [?]: removed value specification. *)

Definition mk_floating_value_pe  (fval : unit )  : generic_pexpr (unit ) (symbol.sym ):= 
  Pexpr [] tt (PEval (Vobject (OVfloating fval))).
(* [?]: removed value specification. *)

Definition mk_nullptr_pe  (ref_ty : ctype )  : generic_pexpr (unit ) (symbol.sym ):= 
  Pexpr [] tt (PEval (Vobject (OVpointer (DAEMON ref_ty)))).
(* [?]: removed value specification. *)
...
Definition mk_member_shift_pe  (pe1 : generic_pexpr (unit ) (symbol.sym )) (tag_sym : symbol.sym ) (member_ident : symbol.identifier )  : generic_pexpr (unit ) (symbol.sym ):= 
...
Definition mk_neg_pe  (pe : generic_pexpr (unit ) (symbol.sym ))  : generic_pexpr (unit ) (symbol.sym ):= 
  Pexpr [] tt (PEop OpSub (mk_integer_pe(Coq.ZArith.BinIntDef.Z.of_nat 0)) pe).
(* [?]: removed value specification. *)

Definition mk_struct_pe  (tag_sym : symbol.sym ) (xs : list ((symbol.identifier *generic_pexpr (unit ) (symbol.sym )) % type))  : generic_pexpr (unit ) (symbol.sym ):= 
  Pexpr [] tt (PEstruct tag_sym xs).
(* [?]: removed value specification. *)

Definition mk_union_pe  (tag_sym : symbol.sym ) (memb_ident : symbol.identifier ) (pe : generic_pexpr (unit ) (symbol.sym ))  : generic_pexpr (unit ) (symbol.sym ):= 
  Pexpr [] tt (PEunion tag_sym memb_ident pe).
(* [?]: removed value specification. *)

Definition mk_memberof_pe  (tag_sym : symbol.sym ) (memb_ident : symbol.identifier ) (pe : generic_pexpr (unit ) (symbol.sym ))  : generic_pexpr (unit ) (symbol.sym ):= 
...
Definition pstore {a b c : Type} `{Default c}  (loc : unit ) (ty : generic_pexpr b a) (x : generic_pexpr b a) (n : generic_pexpr b a) (mo1 : memory_order.memory_order )  : generic_expr c b a:= 
  Expr [] (Eaction (Paction Pos (Action loc default (Store false ty x n mo1)))).
Definition pstore_lock {a b c : Type} `{Default c}  (loc : unit ) (ty : generic_pexpr b a) (x : generic_pexpr b a) (n : generic_pexpr b a) (mo1 : memory_order.memory_order )  : generic_expr c b a:= 
  Expr [] (Eaction (Paction Pos (Action loc default (Store true ty x n mo1)))).
Definition pload {a b c : Type} `{Default c}  (loc : unit ) (ty : generic_pexpr b a) (x : generic_pexpr b a) (mo1 : memory_order.memory_order )  : generic_expr c b a:= 
  Expr [] (Eaction (Paction Pos (Action loc default (Load ty x mo1)))).
Definition prmw {a b c : Type} `{Default c}  (loc : unit ) (ty : generic_pexpr b a) (x : generic_pexpr b a) (n1 : generic_pexpr b a) (n2 : generic_pexpr b a) (mo1 : memory_order.memory_order ) (mo2 : memory_order.memory_order )  : generic_expr c b a:= 
  Expr [] (Eaction (Paction Pos (Action loc default (RMW ty x n1 n2 mo1 mo2)))).
Definition pcompare_exchange_strong {a b c : Type} `{Default c}  (loc : unit ) (ty : generic_pexpr b a) (x : generic_pexpr b a) (n1 : generic_pexpr b a) (n2 : generic_pexpr b a) (mo1 : memory_order.memory_order ) (mo2 : memory_order.memory_order )  : generic_expr c b a:= 
  Expr [] (Eaction (Paction Pos (
        Action loc default (CompareExchangeStrong ty x n1 n2 mo1 mo2)))).
Definition pcompare_exchange_weak {a b c : Type} `{Default c}  (loc : unit ) (ty : generic_pexpr b a) (x : generic_pexpr b a) (n1 : generic_pexpr b a) (n2 : generic_pexpr b a) (mo1 : memory_order.memory_order ) (mo2 : memory_order.memory_order )  : generic_expr c b a:= 
  Expr [] (Eaction (Paction Pos (
...
(* see Â§6.7.9#10 *)
Program Fixpoint zeros_aux  (tagDefs1 : fmap (symbol.sym ) (tag_definition )) (c : ctype )  : unit := 
  match ( (tagDefs1,c)) with ( tagDefs1, ( Ctype _ ty)) =>
    let zero_ival := DAEMON (Coq.ZArith.BinIntDef.Z.of_nat 0) in
  match ( ty) with | Basic( Integer ity) => DAEMON ity zero_ival
    | Basic( Floating fty) => DAEMON fty DAEMON | Array elem_ty( Some n) =>
    DAEMON (lem_list.replicate (Z.abs_nat ( n)) (zeros_aux tagDefs1 elem_ty))
  (*
  Mem.array_mval (List.replicate (natFromInteger n) (Mem.integer_value_mval zero_ival))
*)
    | Pointer _ ref_ty => DAEMON ref_ty (DAEMON ref_ty) | Atomic ty' =>
    ...
    match ( (fmap_lookup_by
      ...
      DAEMON tag
        (List.map
           (fun (p : (symbol.identifier *((qualifiers *ctype ) % type)) % type) =>
              match ( (p) ) with ( (ident,  (_,  ty))) =>
                (ident, ty, zeros_aux tagDefs1 ty) end) ident_tys) | _ =>
      DAEMON end | Union tag =>
    match ( (fmap_lookup_by
               (fun (sym1 : symbol.sym ) (sym2 : symbol.sym )=>
                  ordCompare sym1 sym2) tag tagDefs1)) with
          | Some( UnionDef( (ident,  (_,  ty'))::_)) =>
      DAEMON tag ident (zeros_aux tagDefs1 ty') | _ => DAEMON end | _ =>
    DAEMON end end.
(* [?]: removed value specification. *)
...
Program Fixpoint in_pattern  (sym1 : symbol.sym ) (g : generic_pattern (unit ) (symbol.sym ))  : bool := 
  match ( (sym1,g)) with ( sym1, ( Pattern _ pat)) =>
    match ( pat) with | CaseBase (sym_opt,  _) =>
      maybe false (fun (sym' : symbol.sym ) => symbol.symbolEqual0 sym1 sym')
        sym_opt | CaseCtor _ pats' => List.existsb (in_pattern sym1) pats'
    end end.
(* [?]: removed value specification. *)
  ...
  match ( (sym1,cval,g)) with ( sym1,  cval, ( Pexpr annot1 bty pexpr_)) =>
    Pexpr annot1 bty
      match ( pexpr_) with | PEsym sym' =>
        if symbol.symbolEqual0 sym1 sym' then PEval cval else pexpr_
        | PEimpl _ => pexpr_ | PEval _ => pexpr_ | PEconstrained xs =>
        apply PEconstrained
          (List.map
             (fun (p : (mem_common.mem_constraint (unit )*generic_pexpr (unit ) (symbol.sym )) % type) =>
        ...
        PEstruct tag_sym
          (List.map
             (fun (p : (symbol.identifier *generic_pexpr (unit ) (symbol.sym )) % type) =>
    ...
    Expr annot1
      match ( expr_) with | Epure pe => Epure (subst_sym_pexpr sym1 cval pe)
        ...
        Easeq (sym', bTy) (subst_sym_action sym1 cval act1)
          (* begin block *)
          if symbol.symbolEqual0 sym1 sym' then act2 else
      ...
      if List.existsb
           (fun (p : (symbol.sym *((core_base_type *generic_pexpr (unit ) (symbol.sym )) % type)) % type) =>
              match ( (p) ) with ( (z,  _)) => symbol.symbolEqual0 sym1 z end)
           sym_bTy_pes then
        match ( debug.warn []
                  (fun (u : unit ) =>
    ...
    fun (x : generic_action_ (unit ) (symbol.sym )) =>
      match (x) with | Create pe1 pe2 pref =>
        ...
        CreateReadOnly (subst_sym_pexpr sym1 cval pe1)
          ...
          pref | Alloc pe1 pe2 pref =>
        Alloc (subst_sym_pexpr sym1 cval pe1) (subst_sym_pexpr sym1 cval pe2)
          pref | Kill b pe => Kill b (subst_sym_pexpr sym1 cval pe)
        | Store b pe1 pe2 pe3 mo1 =>
        Store b (subst_sym_pexpr sym1 cval pe1)
          (subst_sym_pexpr sym1 cval pe2) (subst_sym_pexpr sym1 cval pe3) 
        mo1 | Load pe1 pe2 mo1 =>
        Load (subst_sym_pexpr sym1 cval pe1) (subst_sym_pexpr sym1 cval pe2)
          mo1 | SeqRMW b pe1 pe2 sym' pe3 =>
        SeqRMW b (subst_sym_pexpr sym1 cval pe1)
          (subst_sym_pexpr sym1 cval pe2) sym'
          (* begin block *)
          if symbol.symbolEqual0 sym1 sym' then pe3 else
            subst_sym_pexpr sym1 cval pe3 (* end block *)
        | RMW pe1 pe2 pe3 pe4 mo1 mo2 =>
        RMW (subst_sym_pexpr sym1 cval pe1) (subst_sym_pexpr sym1 cval pe2)
          (subst_sym_pexpr sym1 cval pe3) (subst_sym_pexpr sym1 cval pe4) 
        mo1 mo2 | Fence mo1 => Fence mo1
  ...
  match ( (g,cval,expr1)) with (( Pattern _ pat),  cval,  expr1) =>
    ...
    expr1 | (CaseCtor Ccons [pat1;  pat2],  Vlist bTy_elem( cval1::cvals)) =>
      ...
      let str_ctor := match ( ctor1) with | Cnil _ => "nil" | Ccons => "cons"
                        ...
                        | Civfromfloat => "ivfromfloat" end in DAEMON end end.
(* [?]: removed value specification. *)
  ...
  match ( (sym1,g,g0)) with
    ...
    Pexpr annot1 bty
      match ( pe_) with | PEsym sym' =>
        if symbol.symbolEqual0 sym1 sym' then pe_' else pe_ | PEimpl _ => pe_
        | PEval _ => pe_ | PEconstrained xs =>
        apply PEconstrained
          (List.map
             (fun (p : (mem_common.mem_constraint (unit )*generic_pexpr (unit ) (symbol.sym )) % type) =>
                match ( (p) ) with ( (constrs,  pe)) =>
    ...
    fun (x : generic_action_ (unit ) (symbol.sym )) =>
      match (x) with | Create pe1 pe2 pref =>
        ...
        CreateReadOnly (unsafe_subst_sym_pexpr a pe' pe1)
          ...
          (unsafe_subst_sym_pexpr a pe' pe3) pref | Alloc pe1 pe2 pref =>
        Alloc (unsafe_subst_sym_pexpr a pe' pe1)
          (unsafe_subst_sym_pexpr a pe' pe2) pref | Kill b pe =>
        Kill b (unsafe_subst_sym_pexpr a pe' pe) | Store b pe1 pe2 pe3 mo1 =>
        Store b (unsafe_subst_sym_pexpr a pe' pe1)
          (unsafe_subst_sym_pexpr a pe' pe2)
          (unsafe_subst_sym_pexpr a pe' pe3) mo1 | Load pe1 pe2 mo1 =>
        Load (unsafe_subst_sym_pexpr a pe' pe1)
        ...
        SeqRMW b (unsafe_subst_sym_pexpr a pe' pe1)
          ...
          (* begin block *)
          if symbol.symbolEqual0 a sym' then pe3 else
            unsafe_subst_sym_pexpr a pe' pe3 (* end block *)
        | RMW pe1 pe2 pe3 pe4 mo1 mo2 =>
        RMW (unsafe_subst_sym_pexpr a pe' pe1)
          (unsafe_subst_sym_pexpr a pe' pe2)
          (unsafe_subst_sym_pexpr a pe' pe3)
          (unsafe_subst_sym_pexpr a pe' pe4) mo1 mo2 | Fence mo1 => Fence mo1
        | CompareExchangeStrong pe1 pe2 pe3 pe4 mo1 mo2 =>
...
Program Fixpoint to_pure {a : Type}  (g : generic_expr a (unit ) (symbol.sym ))  : option (generic_pexpr (unit ) (symbol.sym )) := 
  ...
  match ( expr_) with | Epure pe => Some pe | Ememop _ _ => None
    ...
    | End _ => None | Epar _ => None | Ewait _ => None | Eannot _ _ => DAEMON
    | Eexcluded _ _ => DAEMON
  (*
    | Eloc _ e ->
        to_pure e
    | Estd _ e ->
...
Program Fixpoint find_labeled_continuation2_aux {a : Type}  (acc : fmap (symbol.sym ) ((list (symbol.sym )*generic_expr a (unit ) (symbol.sym )) % type)) (sym1 : symbol.sym ) (g : generic_expr a (unit ) (symbol.sym ))  : fmap (symbol.sym ) ((list (symbol.sym )*generic_expr a (unit ) (symbol.sym )) % type):= 
  match ( (acc,sym1,g)) with ( acc,  sym1, ( Expr annot1 expr_)) =>
    ...
    acc | Esave (sym',  _) sym_bTys e =>
      if symbol.symbolEqual0 sym1 sym' then
...
Definition update_env  (pat : generic_pattern (unit ) (symbol.sym )) (cval : generic_value (symbol.sym ))  : list (fmap (symbol.sym ) (generic_value (symbol.sym ))) -> list (fmap (symbol.sym ) (generic_value (symbol.sym ))):=  
  fun (x : list (fmap (symbol.sym ) (generic_value (symbol.sym )))) =>
    match (x) with | [] => DAEMON | env1::xs =>
      update_env_aux pat cval env1 :: xs end.
