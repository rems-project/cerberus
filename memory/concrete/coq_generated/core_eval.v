           ...
           if negb (beq_nat (List.length params) (List.length arg_cvals)) then
             ...
             apply exception.except_return
               (utils.foldl2
                  ...
                  arg_cvals) | ProcDecl _ _ _ => DAEMON
           | BuiltinDecl _ _ _ => DAEMON | Proc _ _ _ _ => DAEMON end) end.
...
Definition pull_helper {a b : Type}  (pull : generic_pexpr (unit ) (symbol.sym ) -> generic_pexpr (unit ) (symbol.sym )) (pe_cons : list ((a*generic_pexpr (unit ) (symbol.sym )) % type) -> generic_pexpr_ (unit ) b)  (ps: list  ((a * pexpr ) % type))  : generic_pexpr_ (unit ) b:= 
  ...
  fun (acc : sum (list ((mem_common.mem_constraint (unit )*list ((a*generic_pexpr (unit ) (symbol.sym )) % type)) % type)) (list ((a*generic_pexpr (unit ) (symbol.sym )) % type))) (p : (a*generic_pexpr (unit ) (symbol.sym )) % type) =>
    match ( (acc ,p) ) with ( acc ,  (a1,  pe)) =>
      ...
      inl
        (
        List.map
          (fun (p : (((mem_common.mem_constraint (unit )*list ((a*generic_pexpr (unit ) (symbol.sym )) % type)) % type)*((mem_common.mem_constraint (unit )*list ((a*generic_pexpr (unit ) (symbol.sym )) % type)) % type)) % type) =>
        ...
        match ( acc) with | inr pes' => inr ((a1, pe) :: pes') | inl ys =>
          inl
            (List.map
               (fun (p : (mem_common.mem_constraint (unit )*list ((a*generic_pexpr (unit ) (symbol.sym )) % type)) % type) =>
                  match ( (p) ) with ( (cs,  pes)) =>
                    (cs, ((a1, pe') :: pes)) end) ys) end end end
  ) ps (inr [])) with 
  | inl ys =>
      PEconstrained (
        List.map (fun (p : (mem_common.mem_constraint (unit )*list ((a*generic_pexpr (unit ) (symbol.sym )) % type)) % type) =>
  ...
  let wrap_bin := (fun (f : generic_pexpr (unit ) (symbol.sym ) -> generic_pexpr (unit ) (symbol.sym ) -> generic_pexpr_ (unit ) (symbol.sym )) (pe1 : generic_pexpr (unit ) (symbol.sym )) (pe2 : generic_pexpr (unit ) (symbol.sym )) =>
                     match ( (self pe1, self pe2)) with
                       ...
                       PEconstrained
                         (
                         List.map
                           (fun (p : (((mem_common.mem_constraint (unit )*generic_pexpr (unit ) (symbol.sym )) % type)*((mem_common.mem_constraint (unit )*generic_pexpr (unit ) (symbol.sym )) % type)) % type) =>
                              match ( (p) ) with
                                  ( ((cs1,  pe1),  (cs2,  pe2))) =>
  ...
  Pexpr [] tt
    match ( pexpr_) with | PEsym _ => pexpr_ | PEimpl _ => pexpr_
      ...
      PEconstrained
        (
        List.fold_left
          (fun (acc : list ((mem_common.mem_constraint (unit )*generic_pexpr (unit ) (symbol.sym )) % type)) (p : (mem_common.mem_constraint (unit )*generic_pexpr (unit ) (symbol.sym )) % type) =>
    ...
    let aux := fun (x : list (generic_pexpr (unit ) (symbol.sym ))) =>
                 match (x) with | [pe1; pe2; pe3] => PEif pe1 pe2 pe3 | _ =>
                   DAEMON end in wrap_list aux [pe1;pe2;pe3]
      | PEis_scalar pe => wrap PEis_scalar pe | PEis_integer pe =>
      wrap PEis_integer pe | PEis_signed pe => wrap PEis_signed pe
      | PEis_unsigned pe => wrap PEis_unsigned pe | PEbmc_assume pe =>
      wrap PEbmc_assume pe | PEare_compatible pe1 pe2 =>
...
*)
        ...
        | (OpEq,  Some( Vobject( OVinteger ival1)),  Some( Vobject( OVinteger ival2))) =>
            match ( DAEMON mem_st_opt ival1 ival2) with 
              | Some b =>
...
*)
        ...
        | (OpEq,  Some( Vobject( OVfloating fval1)),  Some( Vobject( OVfloating fval2))) =>
            exception_undefined.exception_undef_return (PEval (if DAEMON fval1 fval2 then Vtrue else Vfalse))
        ...
        | (OpGe,  Some( Vobject( OVinteger ival1)),  Some( Vobject( OVinteger ival2))) =>
            (* TODO CHECK *)
            match ( DAEMON mem_st_opt ival2 ival1) with 
              | Some b =>
                  exception_undefined.exception_undef_return (PEval (if b then Vtrue else Vfalse))
              | None =>
                  (* when both outcomes are still possible *)
        ...
        | (OpGe,  Some( Vobject( OVfloating fval1)),  Some( Vobject( OVfloating fval2))) =>
            exception_undefined.exception_undef_return (PEval (if DAEMON fval2 fval1 then Vtrue else Vfalse))
        ...
        | (_,  Some( Vobject( OVinteger ival1)),  Some( Vobject( OVinteger ival2))) =>
            ...
            exception_undefined.exception_undef_return (PEval (Vobject (OVinteger (DAEMON iop ival1 ival2))))
        ...
        | (_,  Some( Vobject( OVfloating fval1)),  Some( Vobject( OVfloating fval2))) =>
            ...
            exception_undefined.exception_undef_return (PEval (Vobject (OVfloating (DAEMON fop fval1 fval2))))
...
*)
              ...
              | _ => 
                  apply exception_undefined.exception_undef_fail (Illformed_program ( String.append"["  (String.append("")  (String.append"] the two operands of /\ should be booleans ==> "  (String.append(DAEMON v1)  (String.append" <-> " (DAEMON v2)))))))
            end
        ...
        | (_,  Some _,  Some _) => 
            apply exception_undefined.exception_undef_fail (Illformed_program ( String.append"["  (String.append("")  (String.append"] ill-typed PEop ==> "
                                         (DAEMON (caux.mk_op_pe binop1 pe1 pe2))))))
        | _ => 
(*
            let () = Debug.print_debug 1 [] (fun () ->
...
*)
        ...
        match ( core_aux.lookup_env sym1 env1) with 
          | None =>
              ...
              match ( (fmap_lookup_by (fun (sym1 : symbol.sym ) (sym2 : symbol.sym )=>ordCompare sym1 sym2) sym1(funs  file1))) with 
                | Some( Proc _ _ _ _) =>
                    exception_undefined.exception_undef_return (PEval (Vobject (OVpointer (DAEMON (ctype.Ctype [] ctype.Void)))))
                | _ =>
  ...
  match ( debug.print_debug ( 2) []
            (fun (u : unit ) =>
               match ( (u) ) with ( tt) =>
                 String.append "PEerror ==> " (DAEMON debug_pe) end)) with
    ...
    | PEctor ctor1 pes => exception_undefined.exception_undef_bind
        ...
        match ( (ctor1, caux.valueFromPexprs pes')) with 
          | (Cnil tt,  Some []) => 
  match ( debug.warn [debug.DB_core_dynamics]
            (fun (u : unit ) =>
               match ( (u) ) with ( tt) =>
    ...
    apply exception_undefined.exception_undef_return
      (PEval (Vlist BTy_unit (*TODO this is wrong*) [])) end
          ...
          | (Civmax,  Some [Vctype ty]) =>
              match ( ctype.unatomic_ ty) with 
                |( ctype.Basic( ctype.Integer ity)) => 
                    apply exception_undefined.exception_undef_return (PEval (Vobject (OVinteger (DAEMON ity))))
                | _ =>
                    DAEMON (* DEBUG *)
              end
          ...
          | (Civsizeof,  Some [Vctype ty]) => 
              apply exception_undefined.exception_undef_return (PEval (Vobject (OVinteger (DAEMON ty))))
          | (Civalignof,  Some [Vctype ty]) => 
              apply exception_undefined.exception_undef_return (PEval (Vobject (OVinteger (DAEMON ty))))
          | (CivCOMPL,  Some [Vctype ty;  Vobject( OVinteger ival)]) =>
              match ( ctype.unatomic_ ty) with 
                |( ctype.Basic( ctype.Integer ity)) => 
                    apply exception_undefined.exception_undef_return (PEval (Vobject (OVinteger (DAEMON ity ival))))
                | _ =>
                    DAEMON
              end
          | (CivAND,  Some [Vctype ty;  Vobject( OVinteger ival1);  Vobject( OVinteger ival2)]) =>
              match ( ctype.unatomic_ ty) with 
                |( ctype.Basic( ctype.Integer ity)) => 
                    apply exception_undefined.exception_undef_return (PEval (Vobject (OVinteger (DAEMON ity ival1 ival2))))
                | _ =>
                    DAEMON
              end
          ...
          | (Cfvfromint,  Some [Vobject( OVinteger ival)]) => 
              apply exception_undefined.exception_undef_return (PEval (Vobject (OVfloating (DAEMON ival))))
          | (Civfromfloat,  Some [Vctype( ctype.Ctype []( ctype.Basic( ctype.Integer ity)));  Vobject( OVfloating fval)]) => 
              apply exception_undefined.exception_undef_return (PEval (Vobject (OVinteger (DAEMON ity fval))))
          ...
          | (_,  Some cvals) =>
              if false (*List.any Caux.isConstrainedValue cvals*) then
                ...
                let xs :=
                  List.fold_left (fun (acc : list ((mem_common.mem_constraint (unit )*list (generic_pexpr (unit ) (symbol.sym ))) % type)) (cval : generic_value (symbol.sym )) =>
(*
                    match cval with
                      | Vconstrained xs ->
...
*)
                    List.map (
  fun (p : (mem_common.mem_constraint (unit )*list (generic_pexpr (unit ) (symbol.sym ))) % type) =>
    match ( (p) ) with ( (cs,  pes)) =>
    ...
    match ( (p) ) with ( (cs,  pes')) =>
      (cs, Pexpr [] tt (PEctor ctor1 pes')) end) xs
              ...
              apply exception_undefined.exception_undef_fail (Illformed_program ( String.append"PEctor: one of the operands was ill-typed ==> " 
                                           (String.append(DAEMON pexpr1)  (String.append" <====> "
                                           (stringFromList DAEMON cvals)))))
          | (_,  None) => 
    ...
    | PEarray_shift pe1 ty pe2 => exception_undefined.exception_undef_bind
        ...
        match ( (caux.valueFromPexpr pe1', caux.valueFromPexpr pe2')) with 
          | (Some( Vobject( OVpointer ptr_val)),  Some( Vobject( OVinteger ival))) => 
              apply exception_undefined.exception_undef_return (PEval (Vobject (OVpointer (DAEMON ptr_val ty ival))))
          | (Some _,  Some _) => 
              apply exception_undefined.exception_undef_fail (Illformed_program ( String.append"PEarray_shift: type error ==> "
                                           (DAEMON (Pexpr [] tt (PEarray_shift pe1' ty pe2')))))
          | _ => 
    ...
    | PEmember_shift pe tag_sym member_ident => exception_undefined.exception_undef_bind
        ...
        match ( caux.valueFromPexpr pe') with 
          | Some( Vobject( OVpointer ptr_val)) => 
              apply exception_undefined.exception_undef_return (PEval (Vobject (OVpointer (DAEMON ptr_val tag_sym member_ident))))
          | Some _ => 
    ...
    exception_undefined.exception_undef_bind
      ...
      (fun (pes' : list (pexpr )) =>
         match ( caux.valueFromPexprs pes') with | Some cvals =>
           let xs := List.map
                       (fun (p : (symbol.identifier *generic_value (symbol.sym )) % type) =>
                          match ( (p) ) with ( (memb_ident,  cval)) =>
                            match ( apply
                              ...
                              match ( fromJust "Core_eval, PEstruct 1"
                                        ((lookupBy
                                            (fun (ident1 : symbol.identifier ) (ident2 : symbol.identifier )=>
                                               match ( ident1) with
                                                   ( symbol.Identifier0 _ str1) =>
                                                 match ( ident2) with
                              ...
                              | ctype.UnionDef _ => DAEMON end end )
                       (lem_list.zip idents cvals) in
         exception_undefined.exception_undef_return
            ...
            ((lookupBy
                (fun (ident1 : symbol.identifier ) (ident2 : symbol.identifier )=>
                   match ( ident1) with ( symbol.Identifier0 _ str1) =>
                     match ( ident2) with ( symbol.Identifier0 _ str2) =>
                           ...
                           String.append "Symbol"
                             (stringFromPair
                                (fun (n : nat )=>
                                   Coq.Numbers.DecimalString.NilZero.string_of_int
                                     (Z.to_int
                                        (Coq.ZArith.BinIntDef.Z.of_nat n)))
                                (fun (x_opt : option (string ) )=>
                                   stringFromMaybe
                                     (fun (s : string )=>
                                        String.append """"
                ...
                (* TODO: not sure about this case! it happens when calling a function pointer casted to void* via: ((void ( * )(void))p)(); *)
                  ...
                  | Some mem_st =>
                      ...
                      | Some sym1 =>
                          case_funptrval sym1
                      | None => 
                          apply exception_undefined.exception_undef_fail (Illformed_program ( String.append("")  (String.append": "  (String.append(Coq.Numbers.DecimalString.NilZero.string_of_int (Z.to_int 
addr)) " does not point to a function"))))
                      end
                  | None => 
                      apply exception_undefined.exception_undef_fail (Illformed_program ( String.append("")  (String.append": "  (String.append(Coq.Numbers.DecimalString.NilZero.string_of_int (Z.to_int 
addr)) " does not point to a function"))))
    ...
    | PEmemberof tag_sym memb_ident pe => exception_undefined.exception_undef_bind
        ...
        match ( caux.valueFromPexpr pe') with 
          | Some( Vobject( OVstruct tag_sym' xs)) =>
              if negb (symbol.symbolEqual0 tag_sym tag_sym') then 
  ...
  fun (p : (symbol.identifier *ctype.ctype *unit ) % type) =>
  ...
  match ( memb_ident) with symbol.Identifier0 _ str => str end))
                | Some mval =>
  ...
  match ( memb_ident) with ( symbol.Identifier0 _ str1) =>
    match ( memb_ident') with ( symbol.Identifier0 _ str2) =>
      negb ((string_equal str1 str2)) end end then
                DAEMON
              else
                exception_undefined.exception_undef_return (PEval ((@ snd _ _) (caux.valueFromMemValue mval)))
          | Some cval => 
              apply exception_undefined.exception_undef_fail (Illformed_program ( String.append"PEmemberof ==> " (DAEMON cval)))
          | None =>
  ...
  match ( pull_constrained ( 0) (Pexpr [] tt pe_')) with
    ...
    match ( debug.print_debug ( 5) []
              (fun (u : unit ) =>
                 match ( (u) ) with ( tt) =>
                   String.append "PEcall ===> " (DAEMON (Pexpr [] tt pe_''))
                 end )) with tt =>
...
*)
  ...
  match ( debug.print_debug ( 5) []
            (fun (u : unit ) =>
               match ( (u) ) with ( tt) =>
                 String.append "Core_eval ==> PEif (_) ==> " (DAEMON pexpr1)
               end )) with tt =>
            ...
            (fun (u : unit ) =>
               match ( (u) ) with ( tt) =>
                 String.append "ENTERING Core_eval.eval_pexpr_aux2 ==> "
                   (DAEMON pe) end )) with tt =>
    match ( let pulled := pull_constrained ( 0) pe in
    match ( debug.print_debug ( 5) []
              (fun (u : unit ) =>
                 match ( (u) ) with ( tt) =>
                   String.append "pulled ==> " (DAEMON pulled) end )) with
        tt => pulled end) with
