...
Definition ail_identifier : Type :=  symbol.sym .
Definition ail_identifier_default: ail_identifier  := sym_default.
...
Inductive constant : Type := 
  ...
  | ConstantStruct:         symbol.sym  ->  list  ((symbol.identifier  * constant ) % type) -> constant 
  | ConstantUnion:          symbol.sym  ->  symbol.identifier  ->  constant  -> constant .
...
Inductive expression_ (a : Type) : Type :=  
  ...
  | AilEoffsetof:  ctype  ->  symbol.identifier  -> expression_ a
  ...
  | AilEstruct:  ail_identifier  ->  list  ((symbol.identifier  * option  (expression  a) ) % type) -> expression_ a (* tag, pairs of member id, expression *)
  | AilEunion:  ail_identifier  ->  symbol.identifier  ->  option  (expression  a)  -> expression_ a
  ...
  | AilEmemberof:  expression  a ->  symbol.identifier  -> expression_ a
  | AilEmemberofptr:  expression  a ->  symbol.identifier  -> expression_ a
  | AilEbuiltin:  ail_builtin  -> expression_ a
  | AilEstr:  stringLiteral  -> expression_ a
  | AilEconst:  constant  -> expression_ a
  | AilEident:  ail_identifier  -> expression_ a
  ...
  | AilEva_start:  expression  a ->  ail_identifier  -> expression_ a
...
Instance x63_Located{a: Type} : Located (expression  a):= {
   locOf   a_expr :=  
...
Definition bindings : Type := 
  list  ((ail_identifier  * ((((unit  * storageDuration  * bool ) % type) * qualifiers  * ctype ) % type)) % type).
...
Inductive statement_ (a : Type) : Type := 
  ...
  | AilSlabel:  ail_identifier  ->  statement  a -> statement_ a
  | AilSgoto:  ail_identifier  -> statement_ a
  | AilSdeclaration:  list  ((ail_identifier  * expression  a) % type) -> statement_ a
...
Instance x62_Located{a: Type} : Located (statement  a):= {
   locOf   stmt :=  match ( stmt) with ( AnnotatedStatement loc _) => loc end
}.


Inductive declaration : Type := 
  | Decl_object:    ((storageDuration  * bool ) % type) ->  qualifiers  ->  ctype  -> declaration 
  | Decl_function:  bool  (* has_prototype *)
                   ...
                   ->  bool  (* is_inline *)
                   ->  bool  -> declaration .
Definition declaration_default: declaration  := Decl_object (storageDuration_default, bool_default) qualifiers_default ctype_default. (* is_Noreturn *)
...
Record sigma (a: Type) : Type := {
  declarations: list  ((ail_identifier  * ((unit  * declaration ) % type)) % type);
  object_definitions: list  ((ail_identifier  * (expression  a)) % type);
  (* the list of identifiers are the parameter names used in the body *)
  function_definitions: list  ((ail_identifier  * ((unit  * list  ail_identifier  * statement  a) % type)) % type);
  static_assertions: list  ((expression  a * stringLiteral ) % type);
  tag_definitions: list  ((ail_identifier  * tag_definition ) % type);
  extern_idmap: fmap  symbol.identifier   ((ail_identifier  * id_kind ) % type)
}.
...
Definition gamma : Type :=  list  ((ail_identifier  * ((((unit  * storageDuration  * bool ) % type) * qualifiers  * ctype ) % type)) % type).
Definition gamma_default: gamma  := DAEMON.

Definition ail_program (a: Type) : Type := ( option  ail_identifier   * sigma  a) % type.
