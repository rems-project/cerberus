...
Inductive action_request2 (a : Type) : Type := 
  ...
  | AllocRequest2:  symbol.prefix  ->  tt  ->  tt  -> 
                    (cmm.aid  ->  tt  ->  a) -> action_request2 a
  
  | CreateRequest2:  symbol.prefix  ->  tt  ->  ctype.ctype  ->  option  tt   -> 
                     (cmm.aid  ->  tt  ->  a) -> action_request2 a
  
  | StoreRequest2:  cmm.memory_order  ->  ctype.ctype  ->  (* is_locking *)bool  ->  tt  ->  tt  -> 
                     (cmm.aid  ->  tt  ->  a) -> action_request2 a
  
  | SeqRMWRequest2:  ctype.ctype  ->  tt  ->  (tt  ->  core_runM  tt ) ->  (cmm.aid  ->  tt  ->  tt  ->  tt  ->  a) -> action_request2 a
  
  | LoadRequest2:  (* the request provide a fresh symbolic name to be used by the concurrency if needed *)
                    cmm.memory_order  ->  ctype.ctype  ->  tt  ->  symbol.sym  -> 
                   (* the argument of the callback is either a concrete value if the memory model
                      ...
                      present LoadRequest *)
                   (cmm.aid  ->  tt  ->  tt  ->  a) -> action_request2 a
  
  (* TODO: RMW, Fence *)
  | KillRequest2:  (* is_dynamic *)bool  ->  tt  ->  (cmm.aid  ->  a) -> action_request2 a.
Definition action_request2_default{a: Type} : action_request2 a := AllocRequest2 prefix_default tt_default tt_default (fun (x19 : cmm.aid ) => (fun (x20 :  tt ) => DAEMON)).
      ...
      (fun (x : dyn_annotation ) =>
         match (x) with | DA_neg id2 exclusion2 fp2 =>
           if lem_list.elem id1 exclusion2 || lem_list.elem id2 exclusion1 then
             false else BOOM fp1 fp2 | DA_pos exclusion2 fp2 =>
           if lem_list.elem id1 exclusion2 then false else BOOM fp1 fp2 end)
      xs2 | DA_pos exclusion1 fp1 =>
    List.existsb
      (fun (x : dyn_annotation ) =>
         match (x) with | DA_neg id2 _ fp2 =>
           if lem_list.elem id2 exclusion1 then false else BOOM fp1 fp2
           | DA_pos _ fp2 => BOOM fp1 fp2 end) xs2 end) xs1.
(* [?]: removed value specification. *)
    ...
    match (x) with | DA_neg n excl fp =>
      String.append "DA_neg("
        (String.append (lem_string_extra.stringFromNat n)
           (String.append ", "
              (String.append
                 (stringFromList lem_string_extra.stringFromNat excl) 
               ")"))) | DA_pos excl fp =>
      String.append "DA_pos("
        (String.append (stringFromList lem_string_extra.stringFromNat excl)
           ")") end in
  ...
  match ( (p)) with ( (fps_acc,  cvals_acc)) =>
    fun (x : list (generic_expr a (unit ) (symbol.sym ))) =>
      match (x) with | [] => Some (fps_acc, List.rev cvals_acc)
        ...
        BOOM ( String.append "Core_run.one_step_unseq_aux ==> " (BOOM e)) end
  end.
(* [?]: removed value specification. *)
        ...
        Some
          match ( valueFromPexprs pes) with | Some cvals =>
            ...
            EVAL "Ememop"
              ...
              (* end block *) end
        | Ecase( Pexpr _ tt( PEconstrained xs)) _ =>
        BOOM "TODO: Core_reduction.one_step, Ecase PEconstrained"
        | Ecase pe pat_es =>
        Some
          match ( valueFromPexpr pe) with | Some cval =>
            ...
            match ( select_case subst_sym_expr cval pat_es) with | Some e' =>
              ...
              ILLTYPED ( String.append "Ecase, mismatched ==> " (BOOM expr1))
            end | None =>
            EVAL "Ecase"
              (* begin block *) state_exception_undefined.stExceptUndef_bind
                                  (eval_pexpr1 pe)
                                ...
                                (fun (x : generic_value (symbol.sym )) =>
                                   match (x) with | Vtrue =>
                                     ...
                                     state_exception_undefined.stExceptUndef_return
                                       (env1, e3) | _ =>
                                     BOOM
                                       "TODO(use the core_runM) ILLTYPED, the first operand of an Eif didn't evaluated to a boolean"
...
Definition act_valueFromPexpr   : generic_pexpr (unit ) (symbol.sym ) -> option (value0 ) :=  
  fun (x : generic_pexpr (unit ) (symbol.sym )) =>
    match (x) with | Pexpr _ _( PEconstrained _) =>
      BOOM "Core_reduction.step_action => PEconstrained" | z =>
...
Inductive action_step : Type := 
  ...
  | ACTION_EVAL:  string  ->  core_runM  (core.action0  core_run_annotation ) -> action_step 
...
Definition step_action  (eval_pexpr1 : generic_pexpr (unit ) (symbol.sym ) -> core_run_state  -> exception.exceptM ((undefined.t0 (generic_pexpr (unit ) (symbol.sym ))*core_run_state ) % type) (errors.core_run_cause )) (is_excluded : option (nat ) ) (g : generic_action (core_run_annotation ) (unit ) (symbol.sym ))  : action_step := 
  ...
  match ( act) with | Create pe1 pe2 pref =>
        ...
        ACTION_ILLTYPED
          ...
          String.append ("")
            (String.append "the value of a create_readonly("
               (String.append (BOOM (ctype.Ctype [] (ctype.unatomic_ ty)))
                  (String.append ") didn't match the lvalue type: "
                     (BOOM cval3)))) (* end block *) end
    ...
    BOOM "TODO: Core_reduction.step_action => Store pe1 pe2 PEconstrained"
    ...
    match ( (act_valueFromPexpr pe1, act_valueFromPexpr pe2, act_valueFromPexpr
      ...
      match ( memValueFromValue (ctype.Ctype [] (ctype.unatomic_ ty)) cval) with
        ...
        ACTION_REQUEST "StoreRequest" loc
          ...
          StoreRequest2 mo1 ty is_locking ptrval mval
            (fun (aid1 : nat ) (fp : tt ) => let dyn_annots :=
      ...
      ACTION_EVAL "first operand of Store"
        (* begin block *) state_exception_undefined.stExceptUndef_bind
                            ...
                            (fun (pe1' : generic_pexpr (unit ) (symbol.sym )) =>
                               state_exception_undefined.stExceptUndef_return
                                 (wrap (Store0 is_locking pe1' pe2 pe3 mo1)))
        (* end block *) | (_,  None,  _) =>
      ...
      ACTION_EVAL "third operand of Store"
        (* begin block *) state_exception_undefined.stExceptUndef_bind
                            ...
                            (fun (pe3' : generic_pexpr (unit ) (symbol.sym )) =>
                               state_exception_undefined.stExceptUndef_return
                                 (wrap (Store0 is_locking pe1 pe2 pe3' mo1)))
        (* end block *) end | Load0 pe1 pe2 mo1 =>
    match ( (act_valueFromPexpr pe1, act_valueFromPexpr pe2)) with
      ...
      ACTION_REQUEST "LoadRequest" loc
        ...
        LoadRequest2 mo1 ty ptrval val_sym
          (fun (aid1 : nat ) (fp : tt ) (mval : tt ) => let dyn_annots :=
             match ( is_excluded) with | None => [DA_pos [] fp]
               | Some excl_id => [DA_neg excl_id [] fp] end in
      ...
      ACTION_EVAL "first operand of Load"
        (* begin block *) state_exception_undefined.stExceptUndef_bind
                            ...
                            (fun (pe1' : generic_pexpr (unit ) (symbol.sym )) =>
                               state_exception_undefined.stExceptUndef_return
                                 (wrap (Load0 pe1' pe2 mo1))) (* end block *)
      | (_,  None) =>
  ...
  List.map
    (fun (x : (context *generic_expr (core_run_annotation ) (unit ) (symbol.sym )) % type) =>
       match (x) with
         ...
         match ( expr_) with | Epure( Pexpr _ _( PEval _)) =>
           BOOM
             ( String.append
                 "Core_run.step_ctx ==> found a value with ctx <> CTX ==> "
                 (String.append (BOOM expr1)
                    (String.append " <--> " (stringFromContext ctx))))
           | Epure pe =>
           ...
           if classical_boolean_equivalence p Neg then
             BOOM
               "TODO(better typing) negative SeqRMW should be forbidden by the typecheck"
           else
                                             ...
                                             state_exception_undefined.stExceptUndef_bind
                                               ...
                                               (fun (n : nat ) =>
                                                  state_exception_undefined.stExceptUndef_return
                                                    ...
                                                    ptrval
                                                      (fun (mval : tt ) =>
                                                         ...
                                                         match ((env  th_st)) with
                                                               | [] =>
                                                           BOOM
                                                             "Core_reduction.step_ctx, SeqRMW ==> env is empty"
                                                           | x :: xs =>
                                                           fmap_add sym1
                                                             ((@ snd _ _)
                                                         ...
                                                         state_exception_undefined.stExceptUndef_bind
                                                           ...
                                                           (fun (cval3 : generic_value (symbol.sym )) =>
                                                              ...
                                                              memValueFromValue
                                                                ...
                                                                | None =>
                                                                BOOM
                                                                  "TODO(use the error the monad) didn't match the lvalue type in SeqRMW"
                                                              end))
                                                      (fun (aid1 : nat ) (fp : tt ) (mval : tt ) (mval' : tt ) =>
                                                         let cval_e :=
                                                         caux.mk_value_e
                                                           ((@ snd _ _)
                                                              (caux.valueFromMemValue
                                                    ...
                                                    (* end block *)) | _ =>
                                             BOOM
                                               "TODO(use the error the monad) illtyped SeqRMW"
                                           end)) (* end block *)
           ...
           match ( act) with Action loc _ _ =>
             (* DEBUG *) match ( p) with | Pos =>
               ...
               match ( break_at_bound_and_sseq ctx) with | NO_BOUND =>
                 BOOM "TODO: NO_BOUND (Neg)"
                 | BOUND_NO_SSEQ ctx_bound ctxA =>
           ...
           match ( step_action eval_pexpr1 None (*not excluded*) act1) with
             ...
             | ACTION_REQUEST debug_str loc _request => BOOM "Easeq"
           (*
                                           ...
                                           BOOM pv
                                             ...
                                             (fun ( _ : option (Z ) ) (addr : Z ) =>
                                                ...
                                                match ( BOOM mem_st pv) with
                                                  ...
                                                  BOOM
                                                    ( String.append
                                                        (lem_string_extra.stringFromInteger
                                                           addr)
                                                        " does not point to a function (Eccall)")
                                                end)
                                             (fun ( _ : unit ) =>
                                                BOOM
                                                  "function pointer has an unspecified value"))
                                      | _ =>
                                      BOOM
                                        "TODO(use core_runM) Eccall illtyped first operand"
                                    end) (* end block *) | Eproc _ _ _ =>
           BOOM "TODO: PROC" | End es => BOOM "TODO: ND"
