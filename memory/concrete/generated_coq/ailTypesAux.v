...
Definition has_flexible_array_member {a : Type}  (sigm : sigma a) (ty : ctype )  : bool := 
  ...
  match ( unatomic_ ty) with 
    | Struct tag_sym =>
        match ( (lookupBy (fun (sym1 : symbol.sym ) (sym2 : symbol.sym )=>match ( (sym1, sym2)) with 
  ...
  match ( fun (_:nat) (_: list domain) (_: unit -> string) := tt ( 5) 
          ...
          false
  end) tag_sym(tag_definitions  sigm))) with 
          ...
          | _ =>
              BOOM "AilTypesAux.has_flexible_array_member: Struct, UnionDef or Nothing"
        end
    | _ =>
        false
end.
...
Definition is_complete {a : Type}  (sigm : sigma a) (ty : ctype )  : bool := 
  ...
  match ( fun (_:nat) (_: list domain) (_: unit -> string) := tt ( 5) 
          ...
          false
  end) sym1(tag_definitions  sigm))) with 
        | Some( StructDef _) =>
            true
        | _ =>
            let _ := fun (_:nat) (_: list domain) (_: unit -> string) := tt( 5) [debug.DB_ail_typing]
                (fun (u : unit ) =>
  match ( (u) ) with ( tt) =>
            ...
            (stringFromList
               (stringFromPair
                  ...
                  (fun (def : tag_definition )=>
                     let string_of_identifiers := (fun (tag_defs : list ((cabs.cabs_identifier *((qualifiers *ctype ) % type)) % type)) =>
                                                     String.append
                                                       (List.fold_left
                                                          (fun (acc : string ) (p : (cabs.cabs_identifier *((qualifiers *ctype ) % type)) % type) =>
                                                             match ( (acc ,p) ) with
                                                                 ( acc ,  (cabs.CabsIdentifier _ str,  _)) =>
                                                               String.append
                                                                 acc
                                                                 (String.append
                                                                    str 
...
Definition le_integer_range  (impl1 : integerImpl.implementation ) (ity1 : integerType ) (ity2 : integerType )  : bool := 
  ...
  | (Enum _,  Enum _) =>
      (* impossible *)
      BOOM "le_integer_range: Enum, Enum"
  
  | (Char,  Size_t) =>
      BOOM "WIP le_integer_range: Char, Size_t"
  | (Char,  Ptrdiff_t) =>
      BOOM "WIP le_integer_range: Char, Ptrdiff_t"
  | (Char,  Enum _) =>
      BOOM "WIP le_integer_range: Char, Enum"
  
  | (Bool,  Size_t) =>
      BOOM "WIP le_integer_range: Bool, Size_t"
  | (Bool,  Ptrdiff_t) =>
      BOOM "WIP le_integer_range: Bool, Ptrdiff_t"
  | (Bool,  Enum _) =>
      (* impossible *)
      BOOM "le_integer_range: Bool, Enum"
  
  | (Signed _,  Size_t) =>
      BOOM "WIP le_integer_range: Signed, Size_t"
  | (Signed _,  Ptrdiff_t) =>
      BOOM "WIP le_integer_range: Signed, Ptrdiff_t"
  | (Signed _,  Enum _) =>
      (* impossible *)
      BOOM "le_integer_range: Signed, Enum"
  
  | (Unsigned _,  Size_t) =>
      BOOM "WIP le_integer_range: Unsigned, Size_t"
  | (Unsigned _,  Ptrdiff_t) =>
      BOOM "WIP le_integer_range: Unsigned, Ptrdiff_t"
  | (Unsigned _,  Enum _) =>
      (* impossible *)
      BOOM "le_integer_range: Unsigned, Enum"
  
  | (Ptrdiff_t,  Char) =>
      BOOM "WIP le_integer_range: Ptrdiff_t, Char"
  | (Ptrdiff_t,  Bool) =>
      BOOM "WIP le_integer_range: Ptrdiff_t, Bool"
  | (Ptrdiff_t,  Signed _) =>
      BOOM "WIP le_integer_range: Ptrdiff_t, Signed"
  | (Ptrdiff_t,  Unsigned _) =>
      BOOM "WIP le_integer_range: Ptrdiff_t, Unsigned"
  | (Ptrdiff_t,  Enum _) =>
      (* impossible *)
      BOOM "le_integer_range: Ptrdiff_t, Enum"
  | (Ptrdiff_t,  Size_t) =>
      BOOM "WIP le_integer_range: Ptrdiff_t, Size_t"
  
  | (Size_t,  Char) =>
      BOOM "WIP le_integer_range: Size_t, Char"
  | (Size_t,  Bool) =>
      BOOM "WIP le_integer_range: Size_t, Bool"
  | (Size_t,  Signed _) =>
      BOOM "WIP le_integer_range: Size_t, Signed"
  | (Size_t,  Unsigned _) =>
      BOOM "WIP le_integer_range: Size_t, Unsigned"
  | (Size_t,  Enum _) =>
      (* impossible *)
      BOOM "le_integer_range: Size_t, Enum"
  | (Size_t,  Ptrdiff_t) =>
      BOOM "WIP le_integer_range: Size_t, Ptrdiff_t"
  
  | (Enum _,  Char) =>
      (* impossible *)
      BOOM "le_integer_range: Enum, Char"
  | (Enum _,  Bool) =>
      (* impossible *)
      BOOM "le_integer_range: Enum, Bool"
  | (Enum _,  Signed _) =>
      (* impossible *)
      BOOM "le_integer_range: Enum, Signed"
  | (Enum _,  Unsigned _) =>
      (* impossible *)
      BOOM "le_integer_range: Enum, Unsigned"
  | (Enum _,  Size_t) =>
      (* impossible *)
      BOOM "le_integer_range: Enum, Size_t"
  | (Enum _,  Ptrdiff_t) =>
      (* impossible *)
      BOOM "le_integer_range: Enum, Ptrdiff_t"

  | (Wchar_t,  _) =>
      BOOM "WIP le_integer_range: wchar_t"
  | (_,  Wchar_t) =>
      BOOM "WIP le_integer_range: wchar_t"
  | (Wint_t,  _) =>
      BOOM "WIP le_integer_range: wint_t"
  | (_,  Wint_t) =>
      BOOM "WIP le_integer_range: wint_t"
  end.
(* [?]: removed value specification. *)
...
Definition corresponding_real_type  (c : ctype )  : option (realFloatingType ) := 
  match ( (c)) with ( Ctype _ ty as cty) =>
    ...
    | Basic (Floating (Complex fty))
        Basic (Floating fty)
*)
        | _ => BOOM "AilTypesAux.corresponding_real_type" end else None end.
(* [?]: removed value specification. *)
...
Definition lvalue_conversion {a : Type}  (S1 : sigma a) (cty : ctype )  : errorM (ctype ):= 
  if is_array cty then
    BOOM "trying to do a lvalue conversion on an array type"
