...
let derive_cap _ _ _ = assert false (* CHERI only *)
let cap_assign_value _ _ = assert false (* CHERI only *)
let null_cap _ = assert false (* CHERI only *)
let intcast _ ival = Either.Right ival
...
let eff_member_shift_ptrval _ _ _ = failwith "Defacto_memory.impl_member_shift_ptrval"
